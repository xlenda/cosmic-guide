...
// Na leitura clássica os graus se falam: os quatro Cincos são o MESMO grau
...
function astroForMinor(suitKey, number) {
  if (number === 1) return `raiz das forças do ${SUIT_ELEMENT_PT[suitKey]}`;
...
// Leitura transversal da TIRAGEM (não da carta): grau repetido e naipe
// dominante. É a pergunta que separa quem estudou de quem lê carta por carta —
// "por que caíram três Cincos?" tem resposta na tradição (o mesmo grau em
// elementos diferentes), e até agora o app não tinha como dizer nada sobre isso.
// Devolve string ou null — nunca fabrica padrão onde não há.
export function getSpreadPattern(cards) {
  ...
  if (repeatedRank && RANK_GRADES[repeatedRank]) {
    const qtd = byNumber[repeatedRank];
    notes.push(`Repetiu o mesmo grau ${qtd} vezes nesta tiragem, e na leitura clássica isso pesa mais que cada carta isolada. ${RANK_GRADES[repeatedRank]}`);
  }

  const majors = cards.filter((c) => c.arcana === 'maior').length;
  if (majors >= 2) {
    notes.push(`${majors} Arcanos Maiores em ${cards.length} cartas: a tradição lê isso como assunto que não está inteiramente nas suas mãos — é pano de fundo, não detalhe de rotina.`);
  }
  ...
  if (dominant) {
    const label = { paus: 'Paus (fogo — ação e desejo)', copas: 'Copas (água — afeto e vínculo)', espadas: 'Espadas (ar — mente e verdade)', ouros: 'Ouros (terra — matéria e corpo)' };
    notes.push(`Tiragem dominada por ${label[dominant]}: o assunto inteiro está concentrado nesse território.`);
  }
