...
export function getSpreadPattern(cards) {
  ...
  if (repeatedRank && RANK_GRADES[repeatedRank]) {
    ...
    notes.push(`Repetiu o mesmo grau ${qtd} vezes nesta tiragem, e a leitura contemporânea da tiragem inteira (séc. XX) pesa isso mais que cada carta isolada. ${RANK_GRADES[repeatedRank]}`);
  }

  const majors = cards.filter((c) => c.arcana === 'maior').length;
  if (majors >= 2) {
    notes.push(`${majors} Arcanos Maiores em ${cards.length} cartas: a leitura contemporânea toma isso como assunto que não está inteiramente nas suas mãos — é pano de fundo, não detalhe de rotina.`);
  }
