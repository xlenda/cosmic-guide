...
// FONTE ÚNICA da sequência (ver getStreak abaixo). Importa lib/streakDays.js e
// NUNCA lib/streak.js: streak.js puxa ./webPush, que importa `react-native`, e
// o grafo deste módulo precisa continuar rodando em node puro nos testes
// (mesma restrição já documentada no CoupleContext sobre lib/funnel.js).
import { getStreakSummary, todayStr, previousDayStr, ACTIVE_DAYS_KEY, ACTIVE_TYPES_KEY } from './streakDays';
import { ONBOARDING_INTENT_KEY, ONBOARDING_PROFILE_KEY } from './onboardingPlan';
/* O prefixo das chaves da Madre Maria embutida, para o "Apagar meus dados"
 * varrer por prefixo em vez de por lista de nomes. Vem do almacen DELA (fonte
 * unica): uma copia da string aqui poderia divergir, e o sintoma de divergir e
 * dado intimo sobrevivendo ao Apagar tudo. O arquivo nao importa nada — nem
 * React, nem AsyncStorage — entao nao pesa neste modulo. */
import { PREFIJO as MADRE_MARIA_PREFIXO } from '../madremaria/lib/almacen';
import {
  ORBI_DIARY_RECORDED_KEY,
  ORBI_HISTORY_KEY,
  ORBI_LEGACY_HISTORY_KEYS,
} from './orbiConversation';
...
// não existe mais — a porcentagem saiu do app; ver lib/synastry.js.)
...
// { lastDate, count, longest } — DERIVADO de lib/streakDays.js (o mesmo mapa
// `cosmic-active-days` que alimenta o card de sequência da Home e os
// Relatórios). NÃO existe mais uma segunda contagem.
//
// Por que mudou (achado real de auditoria, 30/07/2026): a chave
// `gff-streak:${voce}:${amor}` era só LIDA e APAGADA — NENHUM arquivo do app
// jamais escreveu nela (o comentário do topo deste módulo admitia que ela veio
// portada "para que uma futura tela" a usasse). Resultado: o anel gigante do
// Progresso ficava em 0 no dia 1 e no dia 90, as conquistas streak7/streak30
// (lib/activity.js) eram INALCANÇÁVEIS — então "Conquistas (x/10)" nunca
// chegava a 10/10 e a celebração final não existia —, a Retrospectiva sempre
// dizia "0 — sequência mais longa", e a pill do hero da Home dizia "Comecem
// hoje a sequência de vocês" com o card logo abaixo dizendo "4 dias seguidos"
// na MESMA dobra. Derivar (em vez de criar uma terceira contagem que alguém
// teria que lembrar de gravar) é o que garante que os dois números sejam
// sempre o mesmo número.
//
// NINGUÉM PERDE SEQUÊNCIA NA TROCA: se algum aparelho tiver a chave legada
// preenchida (funil web antigo, importação futura), ela vira PISO — o `count`
// só é honrado enquanto aquela sequência ainda estaria viva (último dia = hoje
// ou ontem, mesma regra de "vivo" do computeCurrentStreak), e o `longest`,
// que é recorde histórico e nunca deve regredir, entra sempre.
export async function getStreak(voce, amor) {
  const [legacy, derived] = await Promise.all([
    readJSON(streakKey(voce, amor), null),
    getStreakSummary(),
  ]);

  const legacyCount = Number(legacy?.count) || 0;
  const legacyLongest = Number(legacy?.longest) || 0;
  const legacyLastDate = typeof legacy?.lastDate === 'string' ? legacy.lastDate : null;

  const hoje = todayStr();
  const legacyViva = legacyCount > 0 && (legacyLastDate === hoje || legacyLastDate === previousDayStr(hoje));

  const count = Math.max(derived.currentStreak, legacyViva ? legacyCount : 0);
  return {
    lastDate: derived.lastActiveDay || legacyLastDate,
    count,
    // `longest` nunca pode ficar abaixo do que já foi mostrado: entra o maior
    // entre o recorde derivado, a corrida atual e qualquer recorde legado.
    longest: Math.max(derived.longest, count, legacyLongest, legacyCount),
  };
}
...
export async function deleteAllCoupleData(soloEmail) {
  ...
  // pra ser apagado.
  //
  // A SEQUENCIA SAI JUNTO (01/08/2026). Ela nao estava nesta lista, e a tela
  // PROMETE apagar, com todas as letras, nos tres idiomas: "os nomes, signos,
  // datas e horas de nascimento e a sequencia salva de voces dois"
  // (PrivacyScreen.js) e "nomes, signos, datas e sequencia do casal" /
  // "la racha de la pareja" / "the couple's streak" (profile.delete.text).
  // Promessa escrita que o codigo nao cumpria e pior que bug comum.
  //
  // E vazava: quem apagasse tudo e comecasse de novo — ou emprestasse o
  // aparelho — herdava a sequencia do par anterior, porque cosmic-active-days
  // sobrevivia intacto ao "apagar tudo". A constante vem importada de
  // streakDays em vez de repetir a string aqui: chave duplicada a mao e como
  // esta lista fica desatualizada.
  const keysToRemove = [
    PROFILE_KEY,
    USER_SIGN_KEY,
    MIRROR_BIRTH_A_KEY,
    ...
    ORBI_DIARY_RECORDED_KEY,
    ACTIVE_DAYS_KEY,
    // O MAPA DE TIPOS SAI JUNTO (11/09/2026, apontado pelo revisor adversarial):
    // é o metadado de QUAIS leituras a pessoa fez em CADA dia — o mesmo tipo
    // de vazamento que 'cosmic-active-days' teve em 01/08/2026. E sem apagar,
    // o perfil seguinte herdava o dominante do anterior e o calendário pintava
    // uma cor que não era dele.
    ACTIVE_TYPES_KEY,
    mirrorKeyFor(SECURE_BIRTH_SOLO_KEY),
    mirrorKeyFor(SECURE_BIRTH_CITIES_KEY),
  ];
  // NAO entra aqui, de proposito: cosmic-journal (o Diario, que e texto que a
  // pessoa escreveu) e cosmic-jornada-v1. A tela promete apagar dado de
  // nascimento e sequencia — nao promete apagar o que a pessoa escreveu, e
  // apagar mais do que se prometeu e tao ruim quanto apagar menos.
  ...
  /* AS CHAVES DA MADRE MARIA SAEM POR PREFIXO, e nao por nome (11/09/2026).
   *
   * Esta e a TERCEIRA vez que a mesma classe de defeito aparece neste arquivo:
   * 'cosmic-active-days' sobreviveu ao Apagar tudo em 01/08, 'cosmic-active-types'
   * em 11/09 de manha, e agora as 24 chaves da Madre Maria embutida. As duas
   * primeiras foram consertadas acrescentando uma linha a lista literal acima —
   * e o defeito voltou, porque a lista literal E o defeito: toda chave nova
   * nasce fora dela e ninguem e avisado.
   *
   * A Madre guarda, sob 'mm-hr.', o que a pessoa respondeu no onboarding dela:
   * quem terminou com quem, ha quanto tempo, se houve bloqueio, o nome, a data
   * de nascimento, o diario dos dias. Num aparelho emprestado ou vendido, isso
   * e exatamente o que nao pode ficar.
   *
   * Por prefixo, chave nova da Madre ja nasce coberta. O prefixo vem de
   * madremaria/lib/almacen.js (PREFIJO), e todo acesso dela passa por
   * conPrefijo() — nenhum modulo monta chave por fora.
   *
   * getAllKeys() so pode APAGAR o que casa com este prefixo: nunca varrer o
   * storage inteiro. Chave de outro app (ou do proprio Cosmic) que porventura
   * viva aqui nao e assunto desta funcao. */
  try {
    const todas = await AsyncStorage.getAllKeys();
    for (const chave of todas) {
      if (typeof chave === 'string' && chave.startsWith(MADRE_MARIA_PREFIXO)) {
        keysToRemove.push(chave);
      }
    }
  } catch {
    // getAllKeys falhou: segue com a lista literal. Melhor apagar o que da do
    // que abortar o Apagar tudo inteiro por causa da varredura.
  }
