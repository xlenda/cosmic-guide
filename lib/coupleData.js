...
export async function initiateCheckout(voce, amor, customerEmail) {
...
// nunca lança: sem correlationCode salvo ainda, vira { hasAccess: false }.
// Uma falha TRANSITÓRIA (rede instável, 5xx passageiro do servidor) não pode
// virar "sem acesso" pra quem já pagou — por isso tenta até 3 vezes com um
// pequeno intervalo antes de desistir. Um 4xx (ex.: correlationCode
...
export async function checkSubscriptionStatus(voce, amor) {
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status < 500) return { hasAccess: false };
...
export async function deleteAllCoupleData() {
  ...
  const keysToRemove = [PROFILE_KEY];
