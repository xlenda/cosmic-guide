...
export async function initiateCheckout(voce, amor, customerEmail, plan = 'trial') {
...
export async function initiateSoloCheckout(customerEmail, plan = 'trial') {
  const resp = await fetchWithTimeout(`${API_BASE}/api/checkout/initiate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ customerEmail: customerEmail || undefined, plan }),
  });
  if (!resp.ok) throw new Error('initiate falhou');
  const data = await resp.json();
  if (customerEmail && data?.correlationCode) {
