...
// Nome do espelho AsyncStorage de uma chave do SecureStore. Hoje o
// BirthChartScreen ainda não espelha birthChartSolo/birthChartCities com este
// sufixo (só lib/birthData.js espelha, com nome próprio), mas apagar uma chave
// inexistente é no-op no AsyncStorage — e assim, no dia em que essa tela ganhar
// espelho web (mesmo padrão dos outros), o dado já nasce sendo apagado por
// "apagar meus dados" em vez de virar um vazamento novo.
...
export async function checkSubscriptionStatus(voce, amor) {
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status < 500) return { hasAccess: false, confirmed: true };
      // 5xx: cai para o retry abaixo.
    } catch {
...
export function combineAccessResults({ account, couple, solo } = {}) {
  ...
  // As 5 telas exclusivas de casal continuam exigindo assinatura de CASAL —
  // uma assinatura solo ativa nunca as destrava (ver components/FeatureGate.js).
  const hasCoupleAccess = Boolean((a.available && a.hasCoupleAccess) || casalOK);
...
export async function deleteAllCoupleData(soloEmail) {
  ...
  // USER_SIGN_KEY (o signo do modo solo) só entra na lista quando existe de
  // fato um perfil de casal a apagar — um usuário solo que nunca teve dados de
  // casal não deve ter seu signo apagado por esta função (e o correlationCode
  // do casal, que também é específico do par voce/amor, precisa sair junto
  // para não ressuscitar uma assinatura antiga se o mesmo par se cadastrar de novo).
  ...
  if (profile?.voce && profile?.amor) {
    keysToRemove.push(USER_SIGN_KEY);
