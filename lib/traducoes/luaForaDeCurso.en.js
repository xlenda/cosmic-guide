...
// ---------------------------------------------------------------------------
// THE SCREEN CHROME — everything screens/LunarCalendarScreen.js puts on screen
// ---------------------------------------------------------------------------
// The screen is a SHOWCASE: it displays what the engine exports and writes no
// prose of its own. Labels, titles, buttons and error notices live HERE, in all
// three packs, with the same keys — none of it goes through lib/i18n.js.
//
//   • THE BADGE IS NOT A VERDICT. It says "this rule sees empty road right
//     now", not "today is a free day". The sentence has a subject: what sees is
//     the rule, not the app, and it is not your day being judged.
//   • WHAT SITS INSIDE QUOTE MARKS IS THE MARKET TALKING, never the app.
//
// `locale` is DATA, not text: the screen formats dates and times with it.
// `marca` is the domain, and it is never translated.
const MARCA = 'cosmicguide.cloud';

// The share text. It is a STITCH of engine texts (the state under both rules
// and the disagreement between them), never new prose.
...
const TELA = {
  locale: 'en-US',
  kicker: 'VOID-OF-COURSE MOON',
  titulo: 'The two counts, side by side',
  intro:
    'Every week a screenshot goes around saying the Moon is void of course and that today is no day to settle anything. What almost never travels with it is the main thing: there is no single void-of-course Moon. There are at least two counts, some 1,400 years apart, and they cut the same day into different pieces. This screen works out both and shows both — including when they disagree, which is when the subject gets interesting.',
  porQueDuas:
    'The rule almost every app uses is William Lilly\'s, Christian Astrology, London, 1647, p. 112: the empty stretch holds while the Moon has not changed sign. The oldest rule on record is Greek — Antiochus of Athens, 2nd century, and Porphyry of Tyre, 3rd century — and it measures by the next 30° of the Moon\'s path, with no sign anywhere in it. Neither of them is "the true one": both were written, by different people, centuries apart.',
  naoDamosVeredito:
    'Why this screen does not land on a "free day": picking one of the two counts in silence is what the market does, and that is what makes it look as if there were a verdict about your day. Here both show up with the name of whoever stands behind each one, and what to make of them stays with the reader.',
  calculando: 'Working out both counts from the ephemeris…',
  rotuloReguas: 'THE TWO RULES',
  seloVazio: 'this rule sees empty road right now',
  seloCheio: 'this rule sees no empty road right now',
  rotuloDefende: 'WHO STANDS BEHIND IT',
  rotuloCriterio: 'THE CRITERION',
  rotuloQuemUsa: 'WHO USES IT',
  rotuloJanelaAgora: 'THE WINDOW RIGHT NOW',
  rotuloProximo: 'THE NEXT MEETING',
  rotuloRecibo: 'WHERE THIS RULE COMES FROM',
  rotuloDivergencia: 'THE DISAGREEMENT',
  divergemSim: 'the two disagree right now',
  divergemNao: 'the two agree right now',
  rotuloHoje: 'TODAY\'S WINDOWS',
  hojeIntro:
    'The windows that touch today, one list per rule, on your device clock. When one list comes back full and the other comes back empty, the disagreement stops being a matter for books and becomes your day.',
  semJanelaHoje: 'By this rule there is no window today.',
  janelasIndisponiveis:
    'The sweep of the day did not close inside the precision the app requires, so today\'s list is not shown. Half a list would be worse than no list: it would show "no window" where there is one.',
  faixa: (c) => `from ${c.inicio} to ${c.fim}`,
  abrir: 'See the whole rule',
  fechar: 'Close',
  verFontes: 'Where all of this comes from',
  fecharFontes: 'Close',
  rotuloNoOriginal: 'IN THE ORIGINAL, UNTRANSLATED',
  rotuloParafrase: 'IN OUR WORDS',
  rotuloFontes: 'SOURCES',
  compartilhar: 'Share',
  copiado: 'Text copied — paste it wherever you like.',
  naoCopiou: 'Copying did not work here — select the text and copy it.',
  marca: MARCA,
  indisponivelTitulo: 'No sky to work with right now',
  textoCompartilhavel,
};
