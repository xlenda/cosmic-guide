...
const EXPLICACAO = {
  diurno: (c) =>
    ...
    `Tu Sol estaba a ${formatarGraus(c.graus)} grados de esa línea, medidos en el círculo de los signos, del lado de arriba.` +
    `\n\n` +
  ...
  noturno: (c) =>
    `Toda carta cae de un lado o del otro de esa línea, sin término medio: o el Sol ya había salido y aún no se había puesto, o no. ` +
    `Tu Sol estaba a ${formatarGraus(c.graus)} grados de esa línea, medidos en el círculo de los signos, del lado de abajo.` +
    `\n\n` +
    `Lo que eso hace: la astrología antigua reparte los planetas en dos equipos. Tres juegan de día, tres juegan de noche, y uno cambia de lado según el caso. ` +
...
// ---------------------------------------------------------------------------
// EL CHROME DE LA SECCIÓN — las etiquetas que la pantalla dibuja alrededor
// ---------------------------------------------------------------------------
// Misma forma y mismas claves que el bloque `chrome` de seita.pt.js. Vive aquí,
// y no en lib/i18n.js, porque la secta es una sección dentro de la Carta Astral
// y la pantalla solo pasa el `lang`: ninguna frase nace dentro del componente.
//
// `marca` es el dominio: NO se traduce en ningún idioma.
const CHROME = {
  rotulo: 'NACISTE DE DÍA O DE NOCHE',
  abrir: 'Ver la lectura completa',
  fechar: 'Cerrar',
  limitrofeRotulo: 'POR MUY POCO',
  mercurioTitulo: 'EL ÚNICO QUE CAMBIA DE LADO',
  planetasTitulo: 'LOS SIETE, UNO A UNO',
  grupoNaSeita: 'Jugando en casa',
  grupoForaDaSeita: 'Fuera de su propio equipo',
  grupoIndeterminado: 'Sin lado definido',
  grupoModernos: 'Los tres de los que la tabla nada dice',
  reciboRotulo: 'RECIBO',
  notasTitulo: 'LAS SALVEDADES',
  citacoesTitulo: 'EN PALABRAS DE LA FUENTE',
  fontesTitulo: 'FUENTES',
  indisponivelTitulo: 'Todavía falta un dato para decirlo',
  comoResolverRotulo: 'CÓMO RESOLVERLO',
  compartilhar: 'Compartir',
  copiado: 'Texto copiado — pégalo donde quieras.',
  naoCopiou: 'Aquí el copiado falló — selecciona el texto y cópialo.',
  marca: 'cosmicguide.cloud',
  compartilhavel: (r) =>
    [
      r.chamada,
      'En la fuente antigua esto tiene nombre: secta (hairesis, en griego — el equipo al que pertenece cada planeta). La tabla es de Ptolomeo, Tetrabiblos I.7, s. II, y Vetio Valente lo verifica planeta a planeta en la Anthologiae I.1.',
      r.limitrofe
        ? `La mía salió ${SEITA_NOME[r.seita]}, y por muy poco: el Sol estaba a ${formatarGraus(r.grausDoHorizonte)} grados del horizonte, franja en la que unos minutos cambiarían el resultado.`
        : `La mía salió ${SEITA_NOME[r.seita]}.`,
