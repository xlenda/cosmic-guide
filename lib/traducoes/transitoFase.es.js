...
// ---------------------------------------------------------------------------
// EL CHROME DE LA PANTALLA — todo lo que la VITRINA dibuja alrededor de la
// lectura. Misma forma y mismas claves que el bloque `chrome` de
// transitoFase.pt.js; la pantalla no redacta ni una línea.
// ---------------------------------------------------------------------------
const LINK_COMPARTILHAR = 'https://cosmicguide.cloud/cosmic-guide/';

function tituloDoCard(r) {
  return `${r.transitoRotulo} de hoy × ${r.natalRotulo} de nacimiento`;
}
...
function textoCompartilhavel(r) {
  ...
  return [
    tituloDoCard(r),
    r.linhaCurta,
    `El nombre de esto en la tradición es ${r.faseTermo}.`,
    `${PTOLOMEU}, Tetrabiblos I.24, ${SEC_II} — el capítulo que separa los dos estados.`,
    LINK_COMPARTILHAR,
  ].join('\n');
}
...
const CHROME = {
  ...
  kicker: 'Llegando o yéndose',
  titulo: 'La dirección de los encuentros de hoy',
  intro:
    'Dos cosas pueden estar a la misma distancia de hoy y no tener nada en común: la charla agendada para pasado mañana y la discusión que terminó anteayer. ' +
    'Una todavía va a llegar; la otra ya fue, y lo que queda de ella es rastro. De qué lado está la cosa cambia lo que uno hace con ella — y es justamente eso lo que casi ninguna app dice.',
  porQue:
    'El calendario de arriba marca el día de los encuentros del cielo consigo mismo. Esta parte es otra: mira el cielo de hoy contra tu carta de nacimiento y dice la DIRECCIÓN de cada encuentro. ' +
    'El ángulo que todavía va a cerrarse y el que ya se cerró y se está abriendo aparecen con la MISMA distancia en grados — lo que los separa es el signo de la resta, y ese signo es el que da el verbo.',
  divergencia:
    `Los dos estados tienen nombre desde el ${SEC_II}, y existe un capítulo entero solo sobre ellos: ${PTOLOMEU}, Tetrabiblos I.24, "Of Applications and Separations and the Other Powers" (trad. Robbins). ` +
    `Pero las dos fuentes del mismo siglo no dicen lo mismo. ${PTOLOMEU} define por POSICIÓN — lo que va delante se aplica a lo que sigue —, y eso solo cierra si el planeta avanza siempre hacia adelante. ` +
    `${VALENTE}, Anthologiae IX.3, registra el caso que sobra: un Júpiter en marcha retrógrada "is being carried towards the position of the Ascendant" — yendo hacia el punto, de espaldas. ` +
    `Esta app mide el movimiento y sigue a ${VALENTE}; donde los dos criterios discrepan, el planeta está retrógrado, sin una sola excepción en el barrido que hicimos. La elección queda escrita aquí en vez de escondida.`,
  rotuloLista: 'Uno a uno, con la dirección',
  abrir: 'Ver la lectura entera',
  fechar: 'Cerrar la lectura',
  rotuloRecibo: 'De dónde viene esto',
  rotuloVerbatim: 'Las fuentes, palabra por palabra',
  rotuloNotas: 'Qué es cuenta y qué es lectura',
  compartilhar: 'Compartir',
  copiado: 'Copiado. Solo falta pegarlo donde quieras.',
  naoCopiou: 'No se pudo copiar por aquí. Selecciona el texto de arriba y cópialo a mano.',
  semAspectos:
    'Hoy ningún planeta del cielo está lo bastante cerca de un punto de tu carta de nacimiento para esta lectura. ' +
    'No falta ningún dato: es un día sin encuentro dentro del alcance que usa la app, y mañana la lista cambia sola.',
  semNascimento: {
    texto:
      'Esta parte solo existe con tu fecha de nacimiento: compara el cielo de hoy con tu carta de nacimiento y dice de qué lado del encuentro está cada planeta. ' +
      'Sin la fecha, la app prefiere no mostrar nada antes que mostrar una carta inventada.',
    cta: 'Ingresar en la Carta Astral',
  },
  bloqueado: {
    titulo: 'El primero es abierto. El resto viene con el plan.',
    texto:
      'El encuentro más exacto del día queda siempre abierto aquí. Los demás de tu cielo de hoy, cada uno con su dirección y las fuentes enteras, quedan en la suscripción.',
    cta: 'Ver los planes',
  },
  selo: {
    aplicativo: 'Llegando',
    separativo: 'Yéndose',
    estacionario: 'Detenido',
  },
  tituloDoCard,
  subtituloDoCard,
  textoCompartilhavel,
