...
const EXPLICACAO = {
  diurno: (c) =>
    ...
    `O seu Sol estava a ${formatarGraus(c.graus)} graus dessa linha, medidos no círculo dos signos, do lado de cima.` +
    `\n\n` +
  ...
  noturno: (c) =>
    `Todo mapa cai de um lado ou do outro dessa linha, sem meio-termo: ou o Sol já tinha nascido e ainda não se posto, ou não. ` +
    `O seu Sol estava a ${formatarGraus(c.graus)} graus dessa linha, medidos no círculo dos signos, do lado de baixo.` +
    `\n\n` +
    `O que isso faz: a astrologia antiga divide os planetas em dois times. Três jogam de dia, três jogam de noite, e um troca de lado conforme o caso. ` +
...
// ---------------------------------------------------------------------------
// O CHROME DA SEÇÃO — os rótulos que a tela desenha em volta da leitura
// ---------------------------------------------------------------------------
// Por que isto mora AQUI e não em lib/i18n.js: a seita é uma seção dentro de
// screens/BirthChartScreen.js, e o resto da tela (Casas, Aspectos) ainda tem
// texto cravado em português. Se o chrome da seita saísse do dicionário geral,
// metade da seção falaria o idioma da pessoa e a outra metade não — que é
// exatamente o defeito que a auditoria achou em sete telas. A tela só repassa
// o `lang`; nenhuma frase nasce dentro do componente.
//
// `chrome` e não `tela` porque `tela` já é usado, nos três packs, como o NOME
// da tela onde o dado se informa ('Mapa Astral' / 'Carta Astral' / 'Birth
// Chart') — é o que fecha cada `falta.comoResolver`.
//
// `marca` é o domínio: NÃO se traduz em idioma nenhum.
//
// `compartilhavel` recebe a leitura inteira (o retorno de seitaDoMapa) e
// devolve o texto que sai na folha de compartilhamento. Ele abre pela chamada
// — vida real primeiro —, só depois nomeia a doutrina com obra, autor e
// século, e quando o caso é limítrofe DIZ que foi apertado, em vez de deixar
// o print parecer mais certo do que a conta é.
const CHROME = {
  rotulo: 'NASCEU DE DIA OU DE NOITE',
  abrir: 'Ver a leitura completa',
  fechar: 'Fechar',
  limitrofeRotulo: 'CHAMADA APERTADA',
  mercurioTitulo: 'O ÚNICO QUE TROCA DE LADO',
  planetasTitulo: 'OS SETE, UM A UM',
  grupoNaSeita: 'Jogando em casa',
  grupoForaDaSeita: 'Fora do próprio time',
  grupoIndeterminado: 'Sem lado definido',
  grupoModernos: 'Os três de que a tabela não fala',
  reciboRotulo: 'RECIBO',
  notasTitulo: 'AS RESSALVAS',
  citacoesTitulo: 'NAS PALAVRAS DA FONTE',
  fontesTitulo: 'FONTES',
  indisponivelTitulo: 'Ainda não dá para dizer',
  comoResolverRotulo: 'COMO RESOLVER',
  compartilhar: 'Compartilhar',
  copiado: 'Texto copiado — cola onde quiser.',
  naoCopiou: 'Não deu para copiar aqui — seleciona o texto e copia.',
  marca: 'cosmicguide.cloud',
  compartilhavel: (r) =>
    [
      r.chamada,
      'Na fonte antiga isso tem nome: seita (hairesis, no grego — o time a que cada planeta pertence). A tabela é de Ptolomeu, Tetrabiblos I.7, séc. II, e Vétio Valente confere planeta a planeta na Anthologiae I.1.',
      r.limitrofe
        ? `O meu deu ${SEITA_NOME[r.seita]}, e por pouco: o Sol estava a ${formatarGraus(r.grausDoHorizonte)} graus do horizonte, faixa em que alguns minutos trocariam o resultado.`
        : `O meu deu ${SEITA_NOME[r.seita]}.`,
