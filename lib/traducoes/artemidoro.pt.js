...
// ---------------------------------------------------------------------------
// O CHROME DA TELA — os rótulos de screens/DreamScreen.js
// ---------------------------------------------------------------------------
// Nasceu AQUI, e não em lib/i18n.js, pelo mesmo motivo de todo o resto: a tela
// é vitrine, não redige uma linha e só repassa o `lang`. Cada rótulo abaixo
// existe nos três packs com a MESMA chave — quem acrescentar um aqui é
// obrigado a acrescentar nos outros dois, e test/artemidoro.test.js confere.
//
// As regras de conteúdo valem inteiras também neste bloco: nada de saúde, nada
// de promessa, nada de veredito. E um cuidado próprio daqui: os rótulos
// separam com todas as letras o que é da fonte (rotuloDaFonte) do que é do app
// (rotuloDoApp), porque na tela as duas perguntas ficam lado a lado.
const CHROME = {
  kicker: 'ANTES DE INTERPRETAR',
  titulo: 'Este sonho diz alguma coisa?',
  abrir: 'Abrir as perguntas',
  fechar: 'Fechar',
  perguntasTitulo: 'RESPONDA COM O QUE VOCÊ LEMBRA',
  rotuloDaFonte: 'DA FONTE',
  rotuloDoApp: 'PERGUNTA DO APP',
  limpar: 'Recomeçar as respostas',
  rotuloCamada: 'A CAMADA',
  rotuloEspecie: 'A ESPÉCIE',
  rotuloSemEspecie: 'SEM ESPÉCIE',
  rotuloPorque: 'POR QUE CAIU AQUI',
  rotuloDiscordancia: 'AS DUAS RESPOSTAS PUXAM PARA LADOS DIFERENTES',
  explicacaoAbrir: 'Por que esta pergunta vem antes',
  explicacaoFechar: 'Fechar o texto',
  especiesAbrir: 'Ver as cinco espécies',
  especiesFechar: 'Fechar as cinco',
  especiesTitulo: 'AS CINCO, COMO ELAS ESTÃO NA FONTE',
  exemploRotulo: 'POR EXEMPLO',
  ressalvasAbrir: 'Ler as ressalvas e a bibliografia',
  ressalvasFechar: 'Fechar as ressalvas',
  checklistTitulo: 'O QUE ELE EXIGIA SABER ANTES',
  verbatimTitulo: 'NO ORIGINAL',
  verbatimParafrase: 'EM CONVERSA',
  verbatimElisao: 'SOBRE O CORTE',
  fontesTitulo: 'DE ONDE ISTO VEM',
  compartilhar: 'Compartilhar',
  copiado: 'Texto copiado — é só colar onde você quiser.',
  naoCopiou: 'Não deu para copiar por aqui. Selecione o texto e copie à mão.',
  ponteIA:
    'A leitura por inteligência artificial desta tela é outra coisa, e ela é do app — não de Artemidoro. Esta parte de cima só diz em que gaveta o seu relato cai e o que a fonte exigiria saber antes de qualquer interpretação.',
  marca: 'Cosmic Guide',
};

// O texto que sai no compartilhamento. Ele é montado com o que a classificação
// já devolveu — a tela não escreve uma palavra e não remonta nada por conta.
const TEXTO_COMPARTILHAVEL = (r) => {
  if (!r || !r.disponivel) return null;
  const linhas = [
    'Antes de perguntar o que um sonho quer dizer, vem outra pergunta: este sonho diz alguma coisa?',
    '',
    `A camada: ${r.camadaNome} — ${r.camadaTransliteracao} (${r.camadaGrego})`,
    r.camadaRecibo,
  ];
  if (r.especieNome) {
    linhas.push('', `A espécie: ${r.especieNome} — ${r.especieTransliteracao} (${r.especieGrego})`, r.especieRecibo);
  } else {
    linhas.push('', 'A espécie: não deu para dizer — e o app prefere dizer isso a escolher uma das cinco no chute.');
  }
