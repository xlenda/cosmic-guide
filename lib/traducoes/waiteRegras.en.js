...
// ---------------------------------------------------------------------------
// THE SCREEN CHROME — the labels the shop window needs
// ---------------------------------------------------------------------------
// Same keys as the TELA block in lib/traducoes/waiteRegras.pt.js, no more and
// no fewer. `convite` and `conviteLinha` are the COLLAPSED card, so the two
// standing rules apply there too: HOOK FIRST (no Waite, no year, no chapter at
// that height) and no locking — the hook itself says the draw button has not
// moved. `contando` carries the one template in this feature: `{s}`, the
// seconds left.
const TELA = {
  nome: 'Tarot by Theme',
  convite: 'Four small things before the draw',
  conviteLinha:
    'What separates a draw from a reading happens now, in this chair, before your thumb reaches the button. ' +
    'Four small gestures, under a minute together, and they lock nothing: the draw button stays right below.',
  abrir: 'See the four',
  fechar: 'Close',
  pular: 'Close and draw straight away',
  paraQuemRotulo: 'This reading is',
  paraQuemVoce: 'For me',
  paraQuemOutra: 'For somebody else',
  rotuloGesto: 'WHAT TO DO NOW',
  rotuloPorQue: 'WHY',
  rotuloRecibo: 'WHERE THIS COMES FROM',
  rotuloExemplos: 'Example questions',
  contar: 'Count the twenty seconds',
  contando: 'Counting… {s} to go',
  contada: 'Twenty seconds counted',
  marcar: 'Mark as done',
  marcada: 'Done',
  rotuloVerbatim: 'The sentence, in the book’s own English',
  rotuloDatacao: 'The dates behind this screen',
  rotuloFontes: 'Sources',
  verFontes: 'Show the sources',
  ocultarFontes: 'Hide the sources',
};
