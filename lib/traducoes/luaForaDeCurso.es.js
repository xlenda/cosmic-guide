...
// ---------------------------------------------------------------------------
// EL CHROME DE LA PANTALLA — todo lo que screens/LunarCalendarScreen.js escribe
// ---------------------------------------------------------------------------
// La pantalla es VITRINA: muestra lo que el motor exporta y no redacta ni una
// línea. Rótulos, títulos, botones y avisos de error viven ACÁ, en los tres
// packs, con las mismas claves — nada de esto pasa por lib/i18n.js.
//
//   • EL SELLO NO ES VEREDICTO. Dice «esta regla ve vacío ahora», no «hoy es
//     día libre». La frase tiene sujeto: quien ve es la regla, no la app, y no
//     es tu día lo que se está juzgando.
//   • LO QUE VA ENTRE « » ES CITA DEL MERCADO, nunca la voz de la app.
//
// `locale` es DATO, no texto: con él la pantalla formatea hora y fecha.
// `marca` es el dominio, y no se traduce en ningún idioma.
const MARCA = 'cosmicguide.cloud';

// El texto que sale al compartir. Es COSTURA de textos del motor (el estado con
// las dos reglas y la divergencia entre ellas), nunca prosa nueva.
...
const TELA = {
  locale: 'es',
  kicker: 'LUNA FUERA DE CURSO',
  titulo: 'Las dos cuentas, una al lado de la otra',
  intro:
    'Todas las semanas circula una captura diciendo que la Luna está fuera de curso y que hoy no es día de resolver nada. Lo que casi nunca viene con ella es lo principal: no existe UNA Luna fuera de curso. Existen por lo menos dos cuentas, con unos 1.400 años entre una y otra, y recortan el mismo día en pedazos distintos. Esta pantalla calcula las dos y muestra las dos — incluso cuando no coinciden, que es cuando el asunto se pone interesante.',
  porQueDuas:
    'La regla que usa casi cualquier app es la de William Lilly, Christian Astrology, Londres, 1647, p. 112: el vacío vale mientras la Luna no cambie de signo. La regla más antigua que se conoce es griega — Antíoco de Atenas, siglo II, y Porfirio de Tiro, siglo III — y mide por los 30° siguientes del camino de la Luna, sin mirar ningún signo. Ninguna de las dos es «la verdadera»: las dos fueron escritas, por gente distinta, con siglos entre ellas.',
  naoDamosVeredito:
    'Por qué esta pantalla no termina en un «día libre»: elegir una de las dos cuentas en silencio es lo que hace el mercado, y es eso lo que da la impresión de que hay un veredicto sobre tu día. Acá las dos aparecen con el nombre de quien defiende cada una, y qué hacer con ellas sigue siendo de quien lee.',
  calculando: 'Calculando las dos cuentas con efemérides…',
  rotuloReguas: 'LAS DOS REGLAS',
  seloVazio: 'esta regla ve vacío ahora',
  seloCheio: 'esta regla no ve vacío ahora',
  rotuloDefende: 'QUIÉN LA DEFIENDE',
  rotuloCriterio: 'EL CRITERIO',
  rotuloQuemUsa: 'QUIÉN LA USA',
  rotuloJanelaAgora: 'LA VENTANA DE AHORA',
  rotuloProximo: 'LA PRÓXIMA CITA',
  rotuloRecibo: 'DE DÓNDE SALE ESTA REGLA',
  rotuloDivergencia: 'LA DIVERGENCIA',
  divergemSim: 'las dos no coinciden ahora',
  divergemNao: 'las dos coinciden ahora',
  rotuloHoje: 'LAS VENTANAS DE HOY',
  hojeIntro:
    'Las ventanas que tocan el día de hoy, una lista por regla, en el reloj de tu aparato. Cuando una lista viene llena y la otra viene vacía, la divergencia deja de ser asunto de libro y pasa a ser tu día.',
  semJanelaHoje: 'Con esta regla, hoy no hay ninguna ventana.',
  janelasIndisponiveis:
    'El recorrido del día no cerró dentro de la precisión que la app exige, así que la lista de hoy no aparece. Una lista a medias sería peor que ninguna: mostraría «no hay ventana» donde sí la hay.',
  faixa: (c) => `de ${c.inicio} hasta ${c.fim}`,
  abrir: 'Ver la regla entera',
  fechar: 'Cerrar',
  verFontes: 'De dónde sale todo esto',
  fecharFontes: 'Cerrar',
  rotuloNoOriginal: 'EN EL ORIGINAL, SIN TRADUCIR',
  rotuloParafrase: 'EN NUESTRAS PALABRAS',
  rotuloFontes: 'FUENTES',
  compartilhar: 'Compartir',
  copiado: 'Texto copiado — pegalo donde quieras.',
  naoCopiou: 'No se pudo copiar acá — seleccioná el texto y copialo.',
  marca: MARCA,
  indisponivelTitulo: 'Sin cielo para calcular ahora',
  textoCompartilhavel,
};
