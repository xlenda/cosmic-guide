...
// ---------------------------------------------------------------------------
// THE SCREEN CHROME — the labels of screens/DreamScreen.js
// ---------------------------------------------------------------------------
// It is born HERE, not in lib/i18n.js, for the same reason as everything else:
// the screen is a display case, it writes no copy of its own and only passes
// the `lang` through. Every label exists in all three packs under the SAME key
// — adding one here obliges adding it in the other two, and
// test/artemidoro.test.js checks that.
const CHROME = {
  kicker: 'BEFORE INTERPRETING',
  titulo: 'Does this dream say anything?',
  abrir: 'Open the questions',
  fechar: 'Close',
  perguntasTitulo: 'ANSWER WITH WHAT YOU REMEMBER',
  rotuloDaFonte: 'FROM THE SOURCE',
  rotuloDoApp: 'THE APP’S QUESTION',
  limpar: 'Start the answers over',
  rotuloCamada: 'THE LAYER',
  rotuloEspecie: 'THE SPECIES',
  rotuloSemEspecie: 'NO SPECIES',
  rotuloPorque: 'WHY IT LANDED HERE',
  rotuloDiscordancia: 'YOUR TWO ANSWERS PULL IN DIFFERENT DIRECTIONS',
  explicacaoAbrir: 'Why this question comes first',
  explicacaoFechar: 'Close the text',
  especiesAbrir: 'See the five species',
  especiesFechar: 'Close the five',
  especiesTitulo: 'THE FIVE, EXACTLY AS THE SOURCE HAS THEM',
  exemploRotulo: 'FOR INSTANCE',
  ressalvasAbrir: 'Read the caveats and the bibliography',
  ressalvasFechar: 'Close the caveats',
  checklistTitulo: 'WHAT HE DEMANDED KNOWING FIRST',
  verbatimTitulo: 'IN THE ORIGINAL',
  verbatimParafrase: 'IN PLAIN WORDS',
  verbatimElisao: 'ABOUT THE CUT',
  fontesTitulo: 'WHERE THIS COMES FROM',
  compartilhar: 'Share',
  copiado: 'Text copied — paste it wherever you want.',
  naoCopiou: 'Copying did not work here. Select the text and copy it by hand.',
  ponteIA:
    'The AI reading this screen does is a different thing, and it belongs to the app — not to Artemidorus. The part above only says which drawer your report lands in, and what the source would demand knowing before any interpretation at all.',
  marca: 'Cosmic Guide',
};

// The text that goes out when sharing. It is assembled from what the sorting
// already returned — the screen writes no word of its own.
const TEXTO_COMPARTILHAVEL = (r) => {
  if (!r || !r.disponivel) return null;
  const linhas = [
    'Before asking what a dream means, another question comes first: does this dream say anything?',
    '',
    `The layer: ${r.camadaNome} — ${r.camadaTransliteracao} (${r.camadaGrego})`,
    r.camadaRecibo,
  ];
  if (r.especieNome) {
    linhas.push('', `The species: ${r.especieNome} — ${r.especieTransliteracao} (${r.especieGrego})`, r.especieRecibo);
  } else {
    linhas.push('', 'The species: it could not be said — and the app would rather say so than pick one of the five blind.');
  }
