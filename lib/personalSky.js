...
export function personalSkyToday(birth, max = 3, lang = 'pt') {
  ...
  // Mantém o contrato público histórico desta função: `orbLimit` é metadado
  // novo para o Recibo Cósmico, não um campo que as telas antigas conhecem.
  // A conta e a ordenação ficam numa função pura única logo abaixo; daqui sai
  // exatamente a forma anterior, sem obrigar nenhum consumidor a mudar.
  const calculados = personalSkyFromPositions(natal, today, max, lang);
  if (!calculados) return null;
  return calculados.map(({ orbLimit: _orbLimit, ...aspecto }) => aspecto);
}

/**
 * Cruza duas listas de longitudes já calculadas.
 *
 * Esta é a parte pura de `personalSkyToday`: não lê relógio, armazenamento,
 * rede nem efeméride. O 3S passa as posições de um instante UTC explícito;
 * `personalSkyToday` continua passando o meio-dia do dia local como sempre.
 * Assim há uma única tabela de aspectos, fatores de velocidade e regra de
 * ordenação no app inteiro.
 *
 * Além da forma histórica, cada item traz `orbLimit`, o limite efetivamente
 * aplicado depois do fator do planeta em trânsito. Ele existia na conta, mas
 * não era exposto; o Recibo Cósmico precisa mostrá-lo para não apresentar o
 * orbe como um número sem dono.
 */
export function personalSkyFromPositions(natal, today, max = 3, lang = 'pt') {
  if (!Array.isArray(natal) || !Array.isArray(today)) return null;
  ...
  for (const t of today) {
    ...
    for (const n of natal) {
      ...
      for (const { type, angle, orb } of ASPECTS_TABLE) {
        ...
        const limiteAplicado = orb * factor;
        // O arredondamento só limpa resíduo binário como 4.800000000000001
        // no recibo. A porta usa o valor bruto, portanto a regra não muda.
        const orbLimit = Math.round(limiteAplicado * 1000) / 1000;
        if (delta <= limiteAplicado) {
          found.push({
