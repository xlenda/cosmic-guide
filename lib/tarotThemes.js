...
// por test/tarotVoice.test.js, junto com as saídas douradas PT capturadas
...
const POSITION_KEYS = [
  ...
  'Situação', 'Tensão', 'Próximo passo',
];

function isPastPosition(position) {
  return position === 'Passado';
}

function isPresentPosition(position) {
  return position === 'Presente';
}

function isFuturePosition(position) {
  return position === 'Futuro';
}

function isSituationPosition(position) {
  return position === 'Situação';
}

function isTensionPosition(position) {
  return position === 'Tensão';
}

function isNextStepPosition(position) {
  return position === 'Próximo passo';
}
...
function meaningLine(card, isReversed, position, pack) {
  ...
  const contexto = card.keywords?.slice(1).filter(Boolean).join(', ') || eixo || card.name;
  ...
  if (isReversed) {
    ...
    if (isSituationPosition(position)) {
      return fill(pack.frames.invertidaSituacao, {
        nome: card.name,
        eixoNome,
        contexto,
    ...
    if (isTensionPosition(position)) {
      return fill(pack.frames.invertidaTensao, {
        nome: card.name,
        eixoNome,
        significado: card.reversedMeaning,
      });
    }
    if (isNextStepPosition(position)) {
      return fill(pack.frames.invertidaProximoPasso, {
        nome: card.name,
        eixoNome,
  ...
  if (isSituationPosition(position)) {
    return fill(pack.frames.diretaSituacao, {
      nome: card.name,
      contexto,
      significado: stripPeriod(lowerFirst(card.uprightMeaning)),
    });
  }
  if (isTensionPosition(position)) {
    return fill(pack.frames.diretaTensao, {
      nome: card.name,
      significado: stripPeriod(lowerFirst(card.uprightMeaning)),
      sombra: stripPeriod(lowerFirst(card.reversedMeaning)),
    });
  }
  if (isNextStepPosition(position)) {
    return fill(pack.frames.diretaProximoPasso, {
      nome: card.name,
      significado: stripPeriod(lowerFirst(card.uprightMeaning)),
    });
  }
...
function traditionLine(card, themeKey, position, pack) {
  ...
  if (category === 'maior') {
    ...
    if (isSituationPosition(position) && card.letra) {
      return fill(pack.frames.maiorSituacao, {
        opener, nome: card.name, numero: card.number, letra: card.letra,
      });
    }
    if (isTensionPosition(position) && card.letra) {
      return fill(pack.frames.maiorTensao, {
        opener, nome: card.name, letra: card.letra, atributo: attr,
      });
    }
    if (isNextStepPosition(position) && card.caminho) {
      return fill(pack.frames.maiorProximoPasso, {
        opener, nome: card.name, numero: card.number, caminho: card.caminho,
      });
    }
    if (isPastPosition(position) && card.letra) {
      return fill(pack.frames.maiorPassado, { opener, nome: card.name, numero: card.number, letra: card.letra });
    }
    if (isFuturePosition(position) && card.caminho) {
  ...
  if (card.number === 1) {
    if (isSituationPosition(position)) {
      return fill(pack.frames.asSituacao, { campo, nome: card.name, astro });
    }
    if (isTensionPosition(position)) {
      return fill(pack.frames.asTensao, { campo, nome: card.name, astro });
    }
    if (isNextStepPosition(position)) {
      return fill(pack.frames.asProximoPasso, { campo, nome: card.name, astro });
    }
    if (isPastPosition(position)) {
  ...
  if (card.number >= 11) {
    if (isSituationPosition(position)) {
      return fill(pack.frames.corteSituacao, {
        campo, nome: card.name, astro, papel: role,
      });
    }
    if (isTensionPosition(position)) {
      return fill(pack.frames.corteTensao, {
        campo, nome: card.name, astro, papel: role,
      });
    }
    if (isNextStepPosition(position)) {
      return fill(pack.frames.corteProximoPasso, {
        campo, nome: card.name, astro, papel: role,
      });
    }
    if (isPastPosition(position)) {
  ...
  if (card.tituloGD && astro) {
    if (isSituationPosition(position)) {
      return fill(pack.frames.numeradaSituacao, {
        campo, nome: card.name, tituloGD: card.tituloGD, astro, papel: role,
      });
    }
    if (isTensionPosition(position)) {
      return fill(pack.frames.numeradaTensao, {
        campo, nome: card.name, tituloGD: card.tituloGD, astro, papel: role,
      });
    }
    if (isNextStepPosition(position)) {
      return fill(pack.frames.numeradaProximoPasso, {
        campo, nome: card.name, tituloGD: card.tituloGD, astro, papel: role,
      });
    }
    if (isPastPosition(position)) {
...
// `pack` é obrigatório desde 01/08/2026: o prefixo do Futuro era um literal em
// PORTUGUÊS cravado aqui ("Muda o vetor: …") enquanto os três packs já traziam
// a moldura traduzida — e ela estava morta. Medido pela auditoria: 1.560 de
// 1.560 leituras de Futuro em es+en (78 cartas × 5 temas × 2 orientações × 2
// idiomas) terminavam com o português cru. Ou seja: mesmo depois de a tela
// passar o idioma, a TERCEIRA carta de toda tiragem continuaria bilíngue.
function adviceLine(card, isReversed, position, pack) {
  if (isPastPosition(position) || isSituationPosition(position)) return null;
  const conselho = isTensionPosition(position)
    ? card.conselhoInvertido
    : isReversed ? card.conselhoInvertido : card.conselho;
  ...
  if (isTensionPosition(position)) {
    return fill(pack.frames.conselhoTensao, { nome: card.name, conselho });
  }
  if (isNextStepPosition(position)) {
    return fill(pack.frames.conselhoProximoPasso, { nome: card.name, conselho });
  }
  if (isFuturePosition(position)) return fill(pack.frames.conselhoFuturo, { conselho: lowerFirst(conselho) });
  return conselho;
}
...
export function getThemedMeaning(card, themeKey, isReversed = false, position = null, lang = 'pt') {
  ...
  const advice = adviceLine(c, isReversed, pos, pack);
...
// `lang` desde 01/08/2026. O VERBATIM continua em inglês nos três idiomas de
// propósito (traduzir citação é falsificá-la, mesma regra do latim de Manílio
// em lib/zodiacBody.js) — mas o TÍTULO e a NOTA são texto do app explicando o
// que ele faz com a citação, e esses saíam em português para todo mundo. A
// palavra interpolada "direita"/"invertida" também.
export function getWaiteNote(card, lang = 'pt') {
  if (!card || !card.waite1911) return null;
  const { orientacao, verbatim, nota } = card.waite1911;
  const pack = packFor(lang);
  const w = pack.waite || {};
  const orient = (w.orientacao && w.orientacao[orientacao === 'invertida' ? 'invertida' : 'direta']) || orientacao;
  return {
    titulo: w.titulo ? fill(w.titulo, { orientacao: orient }) : `O que A. E. Waite escreveu em 1911 — a carta ${orient} na lista dele`,
    verbatim,
    // A nota é por carta: o pack guarda por id, e o PT do deck é a reserva.
    nota: (w.notas && w.notas[card.id]) || nota,
  };
}
...
// `lang` desde 01/08/2026. Este bloco ("As três juntas") aparece em TODA
// tiragem completa e era um parágrafo inteiro cravado em português — os sete
// vereditos, a moldura e o rótulo "(carta de planeta — fica fora desta regra)".
// Os três packs já traziam tudo isso traduzido, e nada era usado. Também
// entrava assim no corpo gravado no Diário Cósmico.
export function getElementalDignity(cards, lang = 'pt') {
  ...
  // Nome da carta e do elemento vêm do pack; as CHAVES (fogo/água/ar/terra)
  // continuam em português nos três, porque são identificadores internos.
  const pack = packFor(lang);
  const dg = pack.dignidade;
  const nomeDe = (c) => (getLocalizedCard(c, lang) || c).name;
  const elemDe = (e) => (dg && pack.elements && pack.elements[e]) || e;
  ...
  // Qual dos sete vereditos — a decisão é a MESMA de sempre; só o texto passou
  // a vir do pack em vez de estar cravado aqui.
  let chave;
  if (contrarias === 2) chave = 'duasContrarias';
  else if (contrarias === 1 && iguais + amigas === 1) chave = 'dividida';
  else if (contrarias === 1) chave = 'umaContraria';
  else if (iguais === 2) chave = 'duasIguais';
  else if (iguais + amigas === 2) chave = 'fortalecida';
  else if (iguais + amigas === 1) chave = 'apoioDeUmLado';
  else chave = 'passiva';

  const veredito = fill(dg.vereditos[chave], { nome: nomeDe(center) });

  const umaVizinha = (carta, el) =>
    el
      ? fill(dg.vizinha, { nome: nomeDe(carta), elemento: elemDe(el) })
      : fill(dg.vizinhaForaDaRegra, { nome: nomeDe(carta) });

  const vizinhas = [umaVizinha(left, elL), umaVizinha(right, elR)].join(dg.vizinhasConector);

  return fill(dg.intro, {
    nome: nomeDe(center),
    elemento: elemDe(elC),
    vizinhas,
    veredito,
