...
function traditionLine(card, themeKey, position) {
  ...
  if (card.number === 1) {
    if (position === 'Passado') {
      return `${campo} Golden Dawn: o Ás é ${astro}, e naquele ponto o potencial estava inteiro.`;
    }
    if (position === 'Futuro') {
      return `${campo} Golden Dawn: o Ás é ${astro} — semente, não colheita.`;
    }
    return `${campo} Golden Dawn: o Ás é ${astro}, potencial ainda não gasto.`;
  }
...
export function getWaiteNote(card) {
  ...
  return {
    titulo: `Waite, 1911 — a carta ${orientacao} na lista dele`,
    verbatim,
    nota,
  };
}
...
export function getCardTraditionNote(card) {
  ...
  if (card.letra) {
    ...
    bits.push(classe ? `letra ${card.letra}, ${classe} do Sepher Yetzirah` : `letra ${card.letra}`);
  }
  if (card.caminho) bits.push(`caminho ${card.caminho} da Árvore da Vida`);
