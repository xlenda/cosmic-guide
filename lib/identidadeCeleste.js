...
//   - date/time/city: getAnyBirthData (casal → solo → espelho web)
...
//
// NUNCA FABRICA: sem nascimento salvo em NENHUMA fonte, ou sem `date`, o
...
export async function carregarIdentidade() {
  try {
    // FONTE ÚNICA, não a chave do solo (11/09/2026, medido no navegador): a
    // primeira versão lia 'birthChartSolo' direto e a conta de CASAL — que
    // guarda a data em gff-birth-a, salva pelo Quiz — via o convite mesmo com
    // data preenchida. Foi o dono, que é conta de casal, quem não achou a
    // ficha. getAnyBirthData já resolve na ordem certa (casal → solo →
    // espelho web, com cidade) e é o que o Céu de Hoje usa em produção.
    const salvo = await getAnyBirthData();
