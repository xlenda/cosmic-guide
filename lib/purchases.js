...
// A ÚNICA pergunta que a tela faz: dá pra vender aqui?
export const LOJA_ATIVA = Platform.OS !== 'web' && !!RC_KEY;

// require tardio de propósito: react-native-purchases é módulo nativo e não
// roda na web. Um import estático executaria o módulo no bundle web (ele monta
// um NativeEventEmitter no topo). Aqui ele só é tocado depois de LOJA_ATIVA.
function rc() {
...
export async function carregarLoja(appUserId) {
  ...
  for (const [plano, id] of Object.entries(PACOTE_DO_PLANO)) {
    ...
    if (pkg) porPlano[plano] = { preco: pkg.product?.priceString || '', pkg };
  }
