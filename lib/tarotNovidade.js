...
// exportada — e importar a tela quebraria o lazy, ver cabeçalho). A chave de
// storage espelha lib/featureUsage.js (`feature-used-once-<key>`).
const TAROT_FEATURE_KEY = 'tarot';
const CHAVE_PREVIA_VITALICIA = `feature-used-once-${TAROT_FEATURE_KEY}`;
...
export async function temNovidadeDeTaroHoje(hasAccess) {
  try {
    ...
    //
    // AUDITORIA 09/08/2026: leitura DIRETA da chave, não hasUsedFeatureOnce —
    // aquele helper engole o erro de storage e devolve false ("não usou") de
    // propósito, porque lá o erro não pode TRAVAR a pessoa. Aqui a regra é a
    // inversa (erro não pode PROMETER novidade), e o helper transformava
    // falha de leitura em bolinha acesa apontando pra um paywall. Com o
    // getItem cru, a falha cai no catch de baixo → apagada.
    return (await AsyncStorage.getItem(CHAVE_PREVIA_VITALICIA)) === null;
  } catch {
    return false;
  }
}
