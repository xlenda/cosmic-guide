...
//
//   2. ERRO TIPADO. Cada tela faz `try { fetchAiX() } catch { getMockX() }`. Esse
//      fallback mockado é ótimo pra queda de rede — e é a PIOR resposta possível
//      pro paywall: servir uma leitura enlatada no instante em que a cota
//      acabou entrega de graça exatamente o que acabou de ser negado E ensina a
//      pessoa que o produto pago é aquele texto genérico. Então "sem cota" e
//      "sem rede" deixam de ser o mesmo erro: quando o servidor responde com um
//      `code` conhecido, o que sobe é um AiAccessError, e as telas mandam pro
//      paywall em vez de mockar.
...
async function postAi(path, label, body) {
  ...
  if (!resp.ok) {
    ...
    // 402/401 sem corpo reconhecível continua sendo erro genérico (e portanto
    // cai no mock) — nada de abrir paywall por causa de um proxy no caminho.
...
export async function fetchAiWeeklyInsight(readings) {
  return exigirTituloECorpo(await postAi('/api/weekly-insight', 'weekly-insight', { readings }), 'weekly-insight');
}
