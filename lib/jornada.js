...
//
// TODO(i18n) — PARCIALMENTE FEITO. A migração começou pelos rótulos de
// FEATURES (ver a seção 2): eram os seis piores de deixar quebrados, porque
// entram INTERPOLADOS numa frase já traduzida ("Open Diário Cósmico") e são
// nome de tela do app, não citação. Eles já são chave (`jornada.feature.*`).
//
// O QUE FALTA, na ordem: `nome`/`subtitulo` das trilhas, `nome`/`legenda` das
// medalhas e, por último e por volume, `titulo`/`leitura`/`pergunta`/
// `acao.texto` dos 28 dias. Todos sob o prefixo `jornada.`, com o `id` como
// segmento de chave (camelCase, sem hífen — chave com hífen escapa da varredura
// estática de test/i18nKeysExist.test.js) e a chave escrita LITERAL, nunca
// montada por template, pra varredura enxergar. Enquanto isso não sai,
// screens/HomeScreen.js esconde a "linha de hoje" fora do PT em vez de mostrar
// frase híbrida na primeira tela do app.
//
...
// lib/zodiacBody.js e lib/grounding.js.
//
// ===========================================================================
