...
// `lang` desde 01/08/2026: as 43 datacoes saiam em portugues nos tres idiomas
// — e nao eram so nomes proprios, eram frases de epoca ("sec. II a.C.", "fim
// do sec. XIX"). A FONTE e o que da credibilidade ao app; entrega-la em outra
// lingua no meio de um texto traduzido e o mesmo que entrega-la pela metade.
//
// Nome de OBRA nunca traduz (Tetrabiblos e Tetrabiblos, em qualquer lingua) —
// so o AUTOR ganha a forma consagrada e a datacao ganha a forma da lingua.
export function fontesDoDia(trilhaId, dia, lang = 'pt') {
  const d = diaDaTrilha(trilhaId, dia);
  if (!d) return [];
  return d.fontes.map((id) => {
    const base = DATACOES[id] || {};
    return {
      id,
      ...base,
      autor: traduzirAutor(base.autor, lang),
