...
export const FUNNEL_EVENTS = new Set([
  ...
  'today_line_tap',
  'scratch_reveal',
  // Denúncia de saída de IA (components/ReportarIA.js). Não é degrau de funil —
  // é o canal de sinalização que a política de Conteúdo Gerado por IA do Google
  // Play exige DENTRO do app. Anda por aqui porque a rota já existe, já tem
  // rate-limit, retenção e validação; criar rota nova só pra isso seria uma
  // segunda telemetria pra manter. `kind` = qual superfície, `reason` = motivo.
  'ai_report',
]);

// Espelho de PROPS_KEYS do servidor. Chave fora daqui é DESCARTADA (o servidor
// recusaria o lote inteiro por causa dela).
const PROPS_KEYS = new Set(['screen', 'kind', 'plan', 'mode', 'source', 'lang', 'platform', 'reason']);
...
export const funnel = {
  ...
  onboardingStep: (mode, step, action = 'view') =>
    track('onboarding_step', { mode, screen: step, source: action }, { key: `onboarding_step:${mode}:${step}:${action}` }),
  ...
  scratchReveal: (kind, screen) =>
    track('scratch_reveal', { kind, screen }, { key: `scratch_reveal:${kind}:${screen}` }),
};
