...
// Data em que a cápsula foi CRIADA (selada) — não confundir com `unlockAt`
// (data futura em que ela abre). Cápsulas novas já trazem `createdAt` (ver
// addCapsule em coupleData.js); para cápsulas antigas já persistidas sem esse
// campo, tenta derivar uma data aproximada a partir do `id` (que é sempre um
// timestamp de Date.now() em string). Se nada disso funcionar, a cápsula
// simplesmente não entra na contagem do período (em vez de quebrar o filtro).
function capsuleCreatedAtIso(c) {
  if (typeof c?.createdAt === 'string' && c.createdAt) return c.createdAt;
  const idNum = Number(c?.id);
  if (Number.isFinite(idNum) && idNum > 0) {
    try {
      return new Date(idNum).toISOString();
    } catch {
      return null;
    }
  }
...
export async function monthlyRecap(voce, amor) {
  ...
  const capsulesSealedThisMonth = (timeline.capsules || []).filter((c) => {
    const createdIso = capsuleCreatedAtIso(c);
    return !!createdIso && createdIso.startsWith(ym);
  }).length;
  const data = await collectData(voce, amor);
  return {
...
export async function yearlyRecap(voce, amor) {
  ...
  const capsulesSealedThisYear = (timeline.capsules || []).filter((c) => {
    const createdIso = capsuleCreatedAtIso(c);
    return !!createdIso && createdIso.slice(0, 4) === year;
  }).length;
