...
import { awardTokens, getTokenBalance } from './tokens';
...
// Onde plugar cada uma está documentado no README da integração/na entrega —
// uma linha por tela, sem tocar em lógica existente.
...
export const MISSION_POOL = [
  ...
  // ---- explorar (3) — sorteia 1/dia
  {
    ...
    verify: { kind: 'action', action: MISSION_ACTIONS.CALENDARIO_LUNAR_VISITADO },
  },
  {
    id: 'horoscopo-do-dia',
    group: 'explorar',
    ...
    verify: { kind: 'action', action: MISSION_ACTIONS.HOROSCOPO_LIDO },
  },
  {
    id: 'conversa-mistica',
    group: 'explorar',
...
// bonusClaimed }. Data diferente de hoje = estado zerado na leitura (virada
// de dia pela mesma convenção local acima); nada de ontem sobrevive, nem os
...
export function completeMission(missionId, now = new Date()) {
  return serialized(async () => {
    ...
    const granted = Math.max(0, Math.min(MISSION_REWARD, MISSIONS_DAILY_TOKEN_CAP - state.tokensEarned));
    state.completed[missionId] = true;
    state.tokensEarned += granted;
    await writeJson(STATE_KEY, state); // flag + crédito do dia juntos, antes do saldo

    if (granted > 0) {
      await awardTokens(granted, `Missão diária: ${mission.title}`);
    }
...
export function claimDailyBonus(now = new Date()) {
  return serialized(async () => {
    ...
    const granted = Math.max(0, Math.min(ALL_DONE_BONUS, MISSIONS_DAILY_TOKEN_CAP - state.tokensEarned));
    state.bonusClaimed = true;
    state.tokensEarned += granted;
    await writeJson(STATE_KEY, state);

    if (granted > 0) {
      await awardTokens(granted, 'Missões diárias: bônus 3 de 3');
    }
