...
// SEIS missões por dia desde 10/09/2026 (3 sorteadas + 3 fixas), não mais 3.
// O teto acompanha sozinho — deixar o 3 cravado aqui faria o app parar de
// pagar na quarta missão sem dizer por quê, que é o pior jeito de errar: a
// pessoa completa, vê o diamante na tela e não recebe.
export const MISSOES_POR_DIA = 6;
export const MISSIONS_DAILY_TOKEN_CAP = MISSOES_POR_DIA * MISSION_REWARD + ALL_DONE_BONUS;
...
export const MISSION_ACTIONS = {
  ...
  // As duas fixas de 10/09/2026. Diferente das de leitura, estas NÃO podem
  // verificar pelo Diário: ver o céu de hoje não gera entrada nenhuma — é
  // leitura de tela, não oráculo guardado. Então voltam ao marcador de ação,
  // disparado no toque do próprio bloco na Home.
  CEU_DE_HOJE_VISTO: 'ceu-de-hoje-visto',
  PROXIMOS_DIAS_VISTO: 'proximos-dias-visto',
};
...
// `arte` (12/09/2026) — a CHAVE do registro de tiles (lib/ilustracoes.js
// TILES), não um require(). A missão manda pra uma tela; a tela já tem
// ilustração pintada na grade da Home; a missão passa a usar A MESMA arte, e
// o olho liga as duas na hora. Nada novo foi gerado: é `tileArte(chave)`, o
// mesmo caminho que CardGrid.js:53 já percorre.
//
// COMPLEMENTAR, NUNCA SUBSTITUIR: `emoji` FICA em todas as treze. Onde não há
// arte correspondente (Pensamento, Frase do Dia, "Complete uma leitura" —
// que é qualquer oráculo, portanto nenhum em particular — e o céu de hoje), a
// missão não declara `arte` e a tela desenha o emoji de sempre. Mesmo
// contrato dos mascotes: chave desconhecida → tileArte devolve null → emoji.
export const MISSION_POOL = [
  ...
  },
  {
    ...
    emoji: '🧭',
    arte: 'chat',
    title: 'Converse com Órbi',
    desc: 'Leve uma pergunta concreta para o guia do app.',
  ...
  // ---- fixas (3) — aparecem TODO DIA, não entram no sorteio
  //
  // Pedido do dono (10/09/2026): "tem 3 missões hoje, colocar a 4ª no mesmo
  // estilo, igual céu de hoje pra você; céu nos próximos dias também, e ver
  // compatibilidade também".
  //
  // As três já existiam como BLOCOS dentro do card de Missões — texto e link,
  // sem recompensa. Viram missão de verdade: mesmo desenho das outras, mesmo
  // diamante. O que muda não é o conteúdo da tela, é o que ela vale.
  //
  // Por que FIXAS e não sorteadas: as três são olhar o céu de hoje, e o céu de
  // hoje é diferente todo dia — não há o que variar. Sortear faria a pessoa
  // perder o que já está na frente dela. Ficam fora de pickFromGroup por isso.
  {
    id: 'ceu-de-hoje',
    group: 'fixa',
    emoji: '🔭',
    title: 'Veja o céu de hoje pra você',
    desc: 'Como o céu de hoje toca o SEU mapa — não o de todo mundo.',
    verify: { kind: 'action', action: MISSION_ACTIONS.CEU_DE_HOJE_VISTO },
  },
  {
    id: 'proximos-dias',
    group: 'fixa',
    emoji: '🗓️',
    arte: 'calendario',
    title: 'Veja o céu nos próximos dias',
    desc: 'O que vem por aí: luas, ingressos e aspectos exatos.',
    verify: { kind: 'action', action: MISSION_ACTIONS.PROXIMOS_DIAS_VISTO },
  },
  {
    id: 'ver-compatibilidade',
    group: 'fixa',
    emoji: '💞',
    arte: 'compatibility',
    title: 'Veja a compatibilidade de vocês',
    desc: 'Os dois signos em cinco áreas da vida, com o aspecto tradicional.',
    verify: { kind: 'journal-type', type: 'compatibility' },
  },
];
...
function todaysMissionDefs(now = new Date()) {
  ...
  return [
    ...
    // AS TRÊS FIXAS entram inteiras, sem sorteio (10/09/2026): o céu de hoje é
    // diferente todo dia, então não há o que variar — sortear só esconderia o
    // que já está na frente da pessoa. Vão no fim porque as sorteadas é que
    // mudam, e mudança merece o topo.
    ...MISSION_POOL.filter((m) => m.group === 'fixa'),
  ];
}
...
export async function getTodaysMissions(now = new Date()) {
  ...
  return defs.map((m) => ({
    ...
    title: m.title,
    desc: m.desc,
    // As chaves de tradução são DERIVADAS do id (13/09/2026). Antes cada
    // missão teria de declarar titleKey/descKey à mão — só uma declarava, e
    // mesmo essa não chegava à tela porque este map não repassava o campo.
    // Derivar aqui fecha os dois buracos de uma vez e torna impossível
    // acrescentar uma missão nova sem tradução: test/missoesIdiomas.test.js
    // exige a chave dos dois lados pra cada id do pool, nos três idiomas.
    titleKey: `missions.${m.id}.title`,
    descKey: `missions.${m.id}.desc`,
