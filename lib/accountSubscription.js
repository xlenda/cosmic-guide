...
import { fetchWithTimeout, setAuthTokenProvider } from './aiClient';
import { setVoiceAuthTokenProvider } from './voiceClient';
import {
  getCorrelationCode,
...
// Limpa uma sessão que o servidor declarou inválida de vez: CORROMPIDA
// (code: token_malformed —
// o valor guardado nem tem forma de JWT). Só é chamada nesse caso: sessão
// expirada é rotina e o SDK renova sozinho; derrubar login por expiração seria
// deslogar gente à toa.
//
// Trava de uma vez por execução: sem ela, duas telas revalidando ao mesmo tempo
// (o refreshAccess roda no foco E de 5 em 5 min) chamariam signOut em paralelo.
let jaLimpouSessao = false;
async function limparSessaoCorrompida() {
  if (jaLimpouSessao) return;
  jaLimpouSessao = true;
  try {
    // scope 'local': só apaga o que está neste aparelho. 'global' revogaria a
    // sessão em todos os aparelhos da pessoa por causa de um storage podre em
    // UM deles — punição desproporcional e do lado errado.
    await supabase.auth.signOut({ scope: 'local' });
  } catch {
    // Nunca lança: se nem o signOut funcionar, o pior caso é o comportamento
    // de antes (revalidar com token ruim), não uma tela quebrada.
  }
}
...
// O CLIENTE DE IA PRECISA DESTE TOKEN (30/07/2026). A cota grátis das leituras
// passou a ser contada no SERVIDOR, por CONTA (server-patches/src/http/aiQuota.js):
// sem o JWT junto da chamada, o backend não tem como saber que é o assinante
// falando e trataria todo mundo como anônimo — o assinante levaria paywall
// depois de pagar, e a cota de quem não assina voltaria a ser burlável só
// trocando de IP.
//
// A ligação é feita AQUI, e não com um import lá dentro, por dois motivos:
// lib/aiClient.js não pode importar este módulo (ele arrasta o Supabase, que
// arrasta react-native, e quebraria test/aiClient.test.js, que roda em
// node:test puro) e o caminho inverso já existe (importamos fetchWithTimeout
// de lá), o que faria um ciclo. Registrar no carregamento do módulo resolve os
// dois: este arquivo entra no bundle pelo CoupleContext, que é montado na raiz
// do App.js, muito antes de qualquer tela conseguir pedir uma leitura.
setAuthTokenProvider(getAuthToken);
// A voz neural usa a mesma identidade verificada das rotas de conta/IA. O
// token só vai no Authorization do nosso backend; nunca é enviado direto ao
// provedor de voz e nenhuma chave de serviço entra no aplicativo.
setVoiceAuthTokenProvider(getAuthToken);
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status === 401 || resp.status === 403) {
        // ...MAS existe um 401 que nunca melhora sozinho: sessão CORROMPIDA.
        // Em produção (30/07/2026) o log pegou 86 "Invalid Compact JWS" num dia,
        // todos do mesmo navegador — o valor guardado nem era um JWT, então
        // renovar não resolvia e o app revalidava a cada 5 min pra sempre.
        // Ficou grave quando a cota de IA passou a depender do token: sessão
        // podre = a pessoa vira anônima, perde a allowlist de dono e vê paywall
        // tendo direito. Aqui a gente limpa a sessão podre UMA vez; o Supabase
        // volta a pedir login e a próxima sessão nasce sã.
        const corpo = await resp.json().catch(() => null);
        // token_malformed: o valor guardado nem tem forma de JWT.
        // account_deleted: a conta foi apagada e o backend revogou o token — o
        // access token so morreria no exp, entao sem isso a pessoa continua vendo
        // uma sessao de uma conta que ela mesma mandou apagar.
        if (corpo?.code === 'token_malformed' || corpo?.code === 'account_deleted') {
          await limparSessaoCorrompida();
        }
        return { ...CONTA_INDEFINIDA, reason: 'unauthorized' };
      }
...
// DELETE /api/subscription/account — apaga do backend próprio o que está ligado
// a ESTA conta (desvincula a assinatura, zera a cota de IA). Roda como SEGUNDO
// passo da exclusão, depois de supabase.rpc('delete_own_account') apagar o
// login (ver ProfileScreen.deleteAccountForReal — o irreversível vem primeiro).
//
// Por isso aceita um token capturado ANTES de a conta morrer: com a conta
// apagada o refresh falha e getSession pode devolver null, mas o JWT já emitido
// continua válido até o exp — o backend o valida por assinatura contra o JWKS
// (server-patches/src/http/socialAuth.js) e a rota só usa o `sub` do payload,
// sem consultar o usuário em lugar nenhum. Sem o parâmetro, cai no
// getAuthToken() de sempre.
//
// Nunca "otimista": qualquer resposta que não seja 2xx volta como ok:false —
// inclusive o 404 de um backend ainda sem a rota. Quem chama é obrigado a dizer
// o que de fato aconteceu; dizer "apagamos" sem ter apagado é exatamente o
// defeito que esta mudança existe pra consertar.
//
// TEM RETRY, e aqui ele vale mais do que no /me: quando esta chamada roda a
// conta JÁ MORREU, não existe "tentar de novo depois" (o app não tem mais
// sessão nenhuma) e o que sobra é uma linha de assinatura apontando pra um uuid
// que não existe mais. Timeout curto de propósito — a pessoa está olhando um
// botão girando depois de uma ação irreversível, então 3 x 8s no pior caso, não
// 3 x 20s. Só repete o que MELHORA repetindo: rede/timeout, 429 e 5xx. 403 de
// e-mail não verificado e 404 de rota não publicada são determinísticos e voltam
// na hora, pra tela poder avisar em vez de fazer a pessoa esperar à toa.
export async function deleteAccountData(tokenDaContaMorta) {
  const token = tokenDaContaMorta || (await getAuthToken());
  if (!token) return { ok: false, reason: 'no-session' };
  let ultimo = { ok: false, reason: 'network' };
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      const resp = await fetchWithTimeout(
        `${API_BASE}/api/subscription/account`,
        { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } },
        8000
      );
      if (resp.ok) return { ok: true };
      ultimo = { ok: false, status: resp.status, reason: `http-${resp.status}` };
      if (resp.status < 500 && resp.status !== 429) return ultimo;
    } catch {
      ultimo = { ok: false, reason: 'network' };
    }
    if (attempt < ATTEMPTS - 1) {
      await new Promise((resolve) => setTimeout(resolve, 500 * (attempt + 1)));
    }
