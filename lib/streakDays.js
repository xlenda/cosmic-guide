...
// TIPO da atividade de cada dia — MAPA PARALELO ao `cosmic-active-days`, nunca
// dentro dele (decisão 11/09/2026): o formato { dia: true|'shield' } é lido
// por computeCurrentStreak, pelos Escudos, pelo Céu de Hoje e por
// test/coupleStreak.test.js, e todos testam `days[key]` como verdadeiro/falso —
// trocar o valor por objeto ou array quebraria a sequência inteira. Aqui fica
// { "2026-09-11": ['tarot', 'dream'] } na ORDEM de gravação: o primeiro do dia
// é o tipo dominante que o calendário dos Relatórios pinta. Dia ativo sem
// entrada aqui (histórico anterior a esta data) fica na cor neutra — nunca se
// inventa um tipo pra ele.
export const ACTIVE_TYPES_KEY = 'cosmic-active-types';

export async function readTypes() {
  try {
    const raw = await AsyncStorage.getItem(ACTIVE_TYPES_KEY);
...
export async function writeTypes(map) {
  try {
    await AsyncStorage.setItem(ACTIVE_TYPES_KEY, JSON.stringify(map));
  } catch {}
}

// Dedup por dia: a mesma leitura repetida (Tarô 3x na terça) conta uma vez só,
// senão o array crescia a cada refresh da tela e o "dominante" perdia o
// sentido. Tipo inválido não grava e não lança — é cosmético, nunca pode
// derrubar quem chamou.
// FILA DE UMA POSIÇÃO (11/09/2026, revisor adversarial): duas telas marcando
// o mesmo dia quase juntas (Home 'horoscope' + ZodiacBody no foco) faziam
// read-modify-write concorrente e a última escrita vencia — um tipo sumia, e
// com ele o dominante do dia. O mapa de dias não sofre disso porque lá a
// escrita é idempotente (days[hoje] = true); aqui a ordem importa. A fila
// encadeia as gravações; erro numa não trava a próxima.
let filaTipos = Promise.resolve();
export function recordActivityType(tipo, dateStr = todayStr()) {
  if (typeof tipo !== 'string' || !tipo) return Promise.resolve();
  const passo = filaTipos.then(async () => {
    const map = await readTypes();
    const atual = Array.isArray(map[dateStr]) ? map[dateStr] : [];
    if (atual.includes(tipo)) return;
    map[dateStr] = [...atual, tipo];
    await writeTypes(map);
  });
  filaTipos = passo.catch(() => {});
  return passo;
}

// Mesma convenção de getMonthActivity (lib/streak.js): month 0-indexed, só as
// chaves do mês pedido. Só devolve arrays — valor corrompido no storage vira
// "sem tipo" na tela em vez de quebrar o `[0]`.
export async function getMonthTypes(year, month) {
  const map = await readTypes();
  const prefix = `${year}-${String(month + 1).padStart(2, '0')}`;
  const result = {};
  Object.keys(map).forEach((d) => {
    if (d.startsWith(prefix) && Array.isArray(map[d])) result[d] = map[d];
  });
  return result;
}
