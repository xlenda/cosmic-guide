...
// lib/rituais.js. O PORTUGUÊS DESTE ARQUIVO É A FONTE DE VERDADE e não mudou um
// byte com os idiomas: os packs de lib/traducoes/mitos.es.js e mitos.en.js
// carregam SÓ texto, na mesma forma, e o motor ganhou `lang` com default 'pt'
// em toda função que devolve texto. Chamada antiga, sem o parâmetro, devolve
// exatamente o que sempre devolveu — test/mitosIdiomas.test.js prova isso
// contra test/golden/mitos.pt.golden.json, capturado ANTES da refatoração.
//
// O CHROME DA TELA MORA AQUI AGORA (CHROME_TELA, mais abaixo). Ele veio de
// screens/MitosScreen.js SEM mudar um caractere, pelo mesmo motivo de
// lib/emocoes.js: lib/i18n.js continua fora do alcance desta fase, então o
// único jeito de a tela falar três idiomas é o chrome viajar pelos packs junto
// com o conteúdo. Quando o integrador migrar tudo pra chave, é daqui que ele
// tira as strings — não de dentro de um componente.
//
...
import PACK_MITOS_ES from './traducoes/mitos.es';
import PACK_MITOS_EN from './traducoes/mitos.en';

const PACKS = { es: PACK_MITOS_ES, en: PACK_MITOS_EN };

// 'pt' (e qualquer idioma desconhecido) devolve null: sem pack, o texto sai da
// fonte de verdade — nunca de uma tradução inventada na hora. É esta linha que
// mantém o PT byte a byte e o golden verde.
...
// ---------------------------------------------------------------------------
// O CHROME DA TELA — em PT, literal, exatamente como estava em screens/
// ---------------------------------------------------------------------------
// Estas quinze strings vieram de screens/MitosScreen.js SEM mudar um caractere
// (o teste de idiomas prova). As duas com contagem viraram MOLDE com {n} e
// {total} em vez de função: template é a única forma que o pack de outro idioma
// consegue carregar, e `preencher` devolve a mesma string de sempre.
//
// `marca` é o domínio — NÃO se traduz em idioma nenhum, é o mesmo endereço nos
// três (mesma regra do LINK_COMPARTILHAR).
...
// Molde → texto. Chave ausente fica como está, à vista, em vez de virar
// "undefined" no meio da frase: erro de moldura tem que aparecer no card, não
// se disfarçar de conteúdo.
...
// O chrome no idioma pedido. Sem idioma (ou com 'pt'), é o objeto literal.
export function chromeDaTela(lang = 'pt') {
  const p = packDoIdioma(lang);
  return (p && p.tela) || CHROME_TELA;
}

// ---------------------------------------------------------------------------
// OS RÓTULOS DO COMPARTILHÁVEL — as três etiquetas do texto que sai do app
// ---------------------------------------------------------------------------
// Ficam separadas do chrome porque não são da tela: elas viajam no WhatsApp de
// quem recebe. Em inglês a tradução literal ("they told you") mata a graça —
// o pack usa "The myth:" / "The source says:" / "Receipts:", que é como um app
// americano escreveria. Ver o cabeçalho de lib/traducoes/mitos.en.js.
export const ROTULOS_COMPARTILHAR = {
  teContaram: 'Te contaram:',
  fonteDiz: 'A fonte diz:',
  recibo: 'Recibo:',
};

function rotulosCompartilhar(lang) {
  const p = packDoIdioma(lang);
  return (p && p.compartilhar) || ROTULOS_COMPARTILHAR;
}

// ---------------------------------------------------------------------------
// BUSCA
// ---------------------------------------------------------------------------
// Veste um mito com o pack do idioma. SÓ TEXTO troca: `id` e `base` ficam
// canônicos — o id é a chave do storage de vistos (traduzi-lo apagaria o
// progresso de quem trocasse de idioma) e a `base` é endereço de arquivo em
// docs/tradicao/, não texto de leitura. Sem pack (lang ausente, 'pt' ou id fora
// do pack), devolve o MESMO objeto que recebeu — é isso que mantém o PT
// intocado byte a byte.
function localizarMito(m, lang) {
  if (!m) return m;
  const p = packDoIdioma(lang);
  const tr = p && p.mitos && p.mitos[m.id];
  if (!tr) return m;
  return {
    ...m,
    oQueTeContaram: tr.oQueTeContaram,
    oQueAFonteDiz: tr.oQueAFonteDiz,
    fonte: tr.fonte,
    detalhe: tr.detalhe,
  };
}

export function mitoPorId(id, lang = 'pt') {
  const m = MITOS.find((x) => x.id === id) || null;
  return m ? localizarMito(m, lang) : null;
}

// O catálogo inteiro no idioma, na MESMA ORDEM de MITOS — a tela navega por
// índice, e o índice do mito do dia é calculado sobre esta ordem.
export function mitosParaIdioma(lang = 'pt') {
  return MITOS.map((m) => localizarMito(m, lang));
}
...
export function mitoDoDia(date = new Date(), lang = 'pt') {
  return localizarMito(MITOS[indiceDoMitoDoDia(date)], lang);
}
...
//
// `lang` é opcional e IDEMPOTENTE: passar o id, o mito em PT ou o mito já
// localizado no idioma dá exatamente o mesmo texto, porque a localização se
// resolve pelo `id`, que nunca é traduzido. E o link é o mesmo nos três — é
// endereço, não texto.
export function textoCompartilhavel(mitoOuId, lang = 'pt') {
  const base = typeof mitoOuId === 'string' ? mitoPorId(mitoOuId) : mitoOuId;
  if (!base) return '';
  const m = localizarMito(base, lang);
  const r = rotulosCompartilhar(lang);
  const linhas = [
    `${r.teContaram} ${m.oQueTeContaram}`,
    `${r.fonteDiz} ${m.oQueAFonteDiz}`,
    `${r.recibo} ${m.fonte}.`,
    LINK_COMPARTILHAR,
  ];
  return linhas.join('\n');
