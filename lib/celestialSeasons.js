...
function nextLunarMilestone(fromDate, horizon = 7) {
  ...
  // A BUSCA COMEÇA NA MEIA-NOITE DE HOJE, NÃO EM `fromDate` (03/08/2026).
  //
  // SearchMoonPhase procura o PRÓXIMO instante depois do que a gente der. Com
  // `fromDate` sendo "agora", uma Lua Cheia às 08:38 já tinha passado quando a
  // pessoa abria o app às 10h — e o app pulava pra a do mês seguinte, fora do
  // horizonte de 7 dias. Resultado: o banner não dizia nada JUSTO no dia do
  // evento, que é o único dia em que ele importa.
  //
  // Medido contra a efeméride real: 21 dos 25 marcos lunares de 2026 sumiam no
  // próprio dia. Não era um caso de borda — era quase todo mês, duas vezes.
  //
  // Partir da meia-noite local devolve o marco de hoje mesmo já passado, e
  // `inDays` (calculado em dias de calendário, logo abaixo) continua dando 0 →
  // "hoje". Nenhum evento de ONTEM entra: a meia-noite de hoje é o piso.
  const event = nextExactNewOrFullMoon(new Date(startOfDay(fromDate)), horizon + 3);
