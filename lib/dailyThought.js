...
export function getThoughtForDate(date = new Date(), personalSign = null) {
  ...
  // de cálculo.
  const aspectWhen = aspect && aspectTier(aspect) === 0 ? 'hoje' : 'neste período';
  const aspectPart = aspect
    ? ` ${aspect.planetA} e ${aspect.planetB} estão em ${aspect.aspectType.toLowerCase()} ${aspectWhen} — ${ASPECT_TONE[aspect.aspectType]}.`
    : '';
...
export function getTodaysThought(personalSign = null, date = new Date()) {
  const abertura = ABERTURA_POR_PERIODO[periodoDoDia(date)];
  return `${abertura} ${getThoughtForDate(date, personalSign)}`;
}
