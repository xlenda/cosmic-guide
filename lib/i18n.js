...
const PLAY_STORE_I18N = {
  pt: {
    ...
    'privacy.rights.sharing': 'Saber com quem compartilhamos: Anthropic (gera as leituras de IA), ElevenLabs (transforma o texto da leitura em áudio quando você toca em Ouvir), Hotmart (cobra a assinatura), Supabase (guarda o login), Google Login (só se você escolher entrar com a conta Google), Vercel e os provedores que hospedam e entregam a versão web e, só quando a medição de anúncio está ligada, Meta (recebe a confirmação da compra com o valor e o seu e-mail embaralhado em hash) e Google Analytics (mede visitas e início de checkout na web). A contagem diária de leituras e o feed social ficam no nosso servidor e não vão para mais ninguém.',
  ...
  es: {
    ...
    'privacy.rights.sharing': 'Saber con quién compartimos: Anthropic (genera las lecturas de IA), ElevenLabs (convierte el texto de la lectura en audio cuando tocas Escuchar), Hotmart (cobra la suscripción), Supabase (guarda el acceso), Google Login (solo si eliges entrar con la cuenta de Google), Vercel y los proveedores que alojan y entregan la versión web y, solo cuando la medición de anuncios está activada, Meta (recibe la confirmación de la compra con el valor y tu correo cifrado en hash) y Google Analytics (mide visitas e inicio de checkout en la web). El conteo diario de lecturas y el feed social quedan en nuestro servidor y no van a nadie más.',
  ...
  en: {
    ...
    'privacy.rights.sharing': 'Know who we share it with: Anthropic (generates the AI readings), ElevenLabs (turns reading text into audio when you tap Listen), Hotmart (bills the subscription), Supabase (holds the login), Google Login (only if you choose to sign in with a Google account), Vercel and the providers that host and deliver the web version and, only when ad measurement is switched on, Meta (receives the purchase confirmation with the amount and your email as a hash) and Google Analytics (measures visits and checkout starts on the web). The daily reading count and the social feed stay on our own server and go nowhere else.',
...
// O FAQ da Ajuda (help.faq.readings.answer) entra aqui pelo mesmo motivo: era a
// outra superfície que repetia a promessa sem ressalva, e o portão de
// test/privacidadeNaoPromete.test.js agora varre TODAS as chaves atrás dela.
const COSMIC_MEMORY_PRIVACY_I18N = {
  pt: {
    'help.faq.readings.answer': 'As leituras que você salva ficam no Diário Cósmico, neste aparelho, junto com os dados de nascimento. Fotos e textos das leituras de IA passam pelo nosso servidor e pela Anthropic para gerar a resposta; por padrão, nosso servidor não guarda o conteúdo da chamada, e o tratamento da Anthropic segue a política dela. Se você ligar a Memória Cósmica em Perfil, o servidor passa a guardar trechos das novas mensagens que você escreve ao Órbi: ficam até você apagar, e as mais antigas saem sozinhas quando passam de 300. Na conversa com Órbi acompanham a pergunta o signo e as três escolhas, quando estão disponíveis juntos, e — com a Memória Cósmica ligada — até quatro lembranças suas; o Diário e o histórico importado nunca são enviados. O histórico atual da conversa fica neste aparelho. O que você publica no feed social fica no nosso servidor e é visível para outros usuários. A lista completa está em Perfil > Privacidade.',
  ...
  es: {
    'help.faq.readings.answer': 'Las lecturas que guardas quedan en el Diario Cósmico, en este dispositivo, junto con tus datos de nacimiento. Las fotos y los textos de las lecturas de IA pasan por nuestro servidor y por Anthropic para generar la respuesta; por defecto, nuestro servidor no guarda el contenido de la llamada y el tratamiento de Anthropic sigue su propia política. Si activas la Memoria Cósmica en Perfil, el servidor pasa a guardar fragmentos de los mensajes nuevos que escribes a Órbi: quedan hasta que los borres, y los más antiguos salen solos al pasar de 300. En la conversación con Órbi acompañan la pregunta el signo y las tres elecciones, cuando están disponibles juntos, y — con la Memoria Cósmica activada — hasta cuatro recuerdos tuyos; el Diario y el historial importado nunca se envían. El historial actual de la conversación queda en este dispositivo. Lo que publicas en el feed social queda en nuestro servidor y es visible para otros usuarios. La lista completa está en Perfil > Privacidad.',
  ...
  en: {
    'help.faq.readings.answer': 'The readings you save stay in the Cosmic Diary on this device, together with your birth details. Photos and text for AI readings pass through our server and Anthropic to generate the response; by default, our server does not store the call content, and Anthropic handles it under its own policy. If you turn on Cosmic Memory in Profile, the server starts keeping excerpts of the new messages you write to Órbi: they stay until you delete them, and the oldest drop off on their own past 300. In an Órbi conversation the sign and three choices travel with the question when they are available together and, with Cosmic Memory on, up to four of your saved memories travel too; your Diary and imported history are never sent. The current conversation history stays on this device. Social-feed posts stay on our server and are visible to other users. The full list is in Profile > Privacy.',
