...
//
// O QUE FALTA: `titulo` e `intencao` dos 21 rituais (o que a pessoa lê no card)
// e, por último e por volume, `materiais`/`passos`/`cuidados`.
//
...
export function textoCompartilhavel(ritual, t) {
  const r = typeof ritual === 'string' ? ritualPorId(ritual) : ritual;
  if (!r) return '';
  ...
  const linhas = [linha1, linha2, AVISO_ETICO, LINK_COMPARTILHAR];
  return linhas.join('\n');
}
