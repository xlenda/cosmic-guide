...
// a byte a de sempre.
export function rituaisPara(momento, lang, t) {
  // O default `= {}` só cobre `undefined`. `rituaisPara(null)` desestruturava
  // null e estourava TypeError — e null é o que chega de quem ainda não sabe a
  // fase da Lua. Agora devolve lista VAZIA, que é a resposta honesta: todos os
  // 21 rituais pedem fase ou dia, então sem momento conhecido nenhum se
  // encaixa. Lista vazia a tela sabe tratar; TypeError ela não sabe.
  const { faseLua = null, diaSemana = null } = momento || {};
