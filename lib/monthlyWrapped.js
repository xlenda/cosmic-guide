...
const ENERGY_PHRASES = {
  ...
  chat: 'Mês de conversa — o papo com Órbi foi seu ritual mais constante.',
  lunarCalendar: 'Mês lunar — você acompanhou os ciclos da Lua de perto.',
};

// Todo o app marca "dia" pelo dia LOCAL do aparelho (lib/localDay.js —
// journal.date = localISOString, streak = localDayStr) — o recorte de mês
// aqui usa o mesmo prefixo pra nunca divergir da contagem que as outras
// telas mostram. Entradas antigas gravadas em UTC continuam datas válidas:
// no pior caso uma leitura noturna pré-update conta no mês vizinho, aceito
// na migração pra não reescrever histórico.
...
export async function computeMonthlyWrapped(year, month) {
  ...
  monthEntries.forEach((e) => {
    if (!countsByType[e.type]) {
      countsByType[e.type] = {
        type: e.type,
        // A tela traduz o nome atual do Órbi no idioma ativo. Manter aqui o
        // rótulo salvo evita injetar português num agregador sem acesso ao i18n.
