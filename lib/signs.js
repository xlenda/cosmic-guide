// Astrologia do casal — lógica de sinastria/compatibilidade portada de
// c:/tmp/gilfforever/web/lib/signs.es.js (mesma matemática de PAIRS/PCT, mesmos
// limiares de data), com o conteúdo em português já usado no restante do app
// (hub/horóscopo web usam esses mesmos nomes de signo, elementos e textos).
// Módulo puro, síncrono e determinístico — sem localStorage/window/document,
// portanto sem nenhuma dependência de browser a remover para rodar no React Native.
//
// FUSO HORÁRIO (adição de 30/07/2026, sem quebrar nada do que já existia):
// as funções que recebem `utcOffsetHours` (ascendantSign, houses,
// astrocartographyCities) passaram a aceitar TAMBÉM o fuso IANA da cidade —
// ver o helper offsetHorasDe() logo abaixo dos SIGNS. Número continua sendo
// número, byte a byte: quem já tem mapa salvo não vê um grau de diferença.
import { resolveOffsetHours } from "./timezone";

export const SIGNS = [
  { name: "Áries", element: "fogo", emoji: "♈", range: "21/03–19/04" },
  { name: "Touro", element: "terra", emoji: "♉", range: "20/04–20/05" },
  { name: "Gêmeos", element: "ar", emoji: "♊", range: "21/05–20/06" },
  { name: "Câncer", element: "água", emoji: "♋", range: "21/06–22/07" },
  { name: "Leão", element: "fogo", emoji: "♌", range: "23/07–22/08" },
  { name: "Virgem", element: "terra", emoji: "♍", range: "23/08–22/09" },
  { name: "Libra", element: "ar", emoji: "♎", range: "23/09–22/10" },
  { name: "Escorpião", element: "água", emoji: "♏", range: "23/10–21/11" },
  { name: "Sagitário", element: "fogo", emoji: "♐", range: "22/11–21/12" },
  { name: "Capricórnio", element: "terra", emoji: "♑", range: "22/12–19/01" },
  { name: "Aquário", element: "ar", emoji: "♒", range: "20/01–18/02" },
  { name: "Peixes", element: "água", emoji: "♓", range: "19/02–20/03" },
];

export function signByName(name) {
  return SIGNS.find((s) => s.name === name) || null;
}

// ---------------------------------------------------------------------------
// Offset em horas a partir do que a tela tiver na mão. Aceita, na MESMA
// posição de argumento em que antes só cabia um número:
//
//   -3                          número puro (assinatura antiga — intocada)
//   "America/Sao_Paulo"         fuso IANA (novo): resolve o horário de verão
//                               do INSTANTE de nascimento
//   { timezone, utcOffset }     o objeto de cidade inteiro (novo, e o mais
//                               seguro): usa o fuso, e cai no utcOffset fixo
//                               se o aparelho não conhecer aquele fuso
//
// POR QUE ISSO IMPORTA: o Brasil teve horário de verão até 2019, e o
// Ascendente muda de signo a cada ~2h. Varrendo as 48 meias-horas de um dia de
// janeiro em São Paulo com esta própria função, 120 de 240 combinações
// data×hora mudam de signo entre -3 e -2 — metade dos nascimentos do período
// saía errada. Ver lib/timezone.js.
//
// A ORDEM DOS TESTES É O CONTRATO DE COMPATIBILIDADE: número entra e sai igual
// antes de qualquer outra coisa acontecer, e o que não for reconhecido vira 0,
// exatamente como era.
// ---------------------------------------------------------------------------
function offsetHorasDe(zona, dateStr, timeStr) {
  if (typeof zona === "number") return Number.isNaN(zona) ? 0 : zona;
  const r = resolveOffsetHours(zona, dateStr, timeStr);
  return typeof r === "number" && !Number.isNaN(r) ? r : 0;
}

// ---------------------------------------------------------------------------
// Instante UTC REAL de uma hora de PAREDE ("nasci 02:00") — a conversão que
// ascendantSign sempre fez à mão e que agora moonSign/planetPositions/aspects
// também podem fazer.
//
// POR QUE ISSO PRECISOU EXISTIR (medido em 30/07/2026, verificação cética):
// moonSign/planetPositions liam `${dateStr}T${timeStr}:00Z` — ou seja, tratavam
// a hora do relógio da maternidade como se fosse UTC, offset ZERO, para todo
// mundo do planeta. Com o Ascendente agora no instante certo, o mesmo mapa
// passava a ter DOIS instantes de nascimento diferentes: Ascendente às 04:00Z
// e Lua às 02:00Z. Dois caminhos discordando é pior que um errado consistente.
//
// Tamanho do erro, varrendo 14.688 nascimentos (1960–2010) por fuso e
// comparando o signo lunar dos dois instantes:
//
//   São Paulo  ......  5,15% das Luas natais com SIGNO errado (deriva até 1,9°)
//   Nova York  ......  8,07%                                  (até 3,2°)
//   Tóquio     ...... 16,64%                                  (até 5,8°)
//   Auckland   ...... 22,60%                                  (até 8,3°)
//
// E 43,9% das listas de aspectos de nascimento em São Paulo mudavam.
// Caso público conferível: Barack Obama (04/08/1961 19:24 Honolulu, dado AA)
// tem Lua em Gêmeos 3° em qualquer efeméride séria — sem o fuso, este app
// mostrava Touro.
//
// `zona` AUSENTE mantém o comportamento histórico (offset 0), que é o que o
// resto do app quer de propósito: HomeScreen/dailyThought/cosmicSound pedem o
// céu de HOJE ancorado no meio-dia UTC, não o instante de um nascimento.
// ---------------------------------------------------------------------------
function instanteUtcMs(dateStr, timeStr, zona) {
  const hora = timeStr || "12:00";
  const base = new Date(`${dateStr}T${hora}:00Z`).getTime();
  if (Number.isNaN(base)) return NaN;
  if (zona === undefined || zona === null) return base;
  return base - offsetHorasDe(zona, dateStr, hora) * 3600 * 1000;
}

// Valida estritamente uma data "YYYY-MM-DD": rejeita formato errado E datas
// impossíveis do calendário (ex.: "2023-02-29" fora de ano bissexto,
// "2023-04-31") que o construtor Date do JS aceitaria em silêncio "rolando"
// pro mês seguinte, em vez de gerar NaN. Retorna o Date (meia-noite local)
// quando a data é real, ou null caso contrário — nunca fabrica um resultado.
function parseStrictDate(dateStr) {
  if (!dateStr) return null;
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(dateStr);
  if (!m) return null;
  const year = +m[1], month = +m[2], day = +m[3];
  const d = new Date(dateStr + "T00:00");
  if (Number.isNaN(d.getTime())) return null;
  if (d.getFullYear() !== year || d.getMonth() + 1 !== month || d.getDate() !== day) return null;
  return d;
}

// Detecta o signo solar a partir da data de nascimento (YYYY-MM-DD).
//
// O SIGNO É ONDE O SOL ESTÁ, NÃO O QUE A TABELA DIZ.
// Ptolomeu, Tetrabiblos I.22: os signos começam nos equinócios e solstícios —
// que são INSTANTES astronômicos, não datas de calendário. O equinócio de março
// cai entre 19 e 21 conforme o ano bissexto, então nenhuma tabela fixa acerta
// sempre.
//
// Medido nesta base, varrendo 1950-2030 com o próprio astronomy-engine que o
// app já usa pra Lua e Ascendente: a tabela errava 293 dias em 29.585 (0,99%).
// As piores fronteiras: 23/10 errado em 44 dos 81 anos, 21/06 em 42, 20/01 em
// 38. Casos reais — 23/10/2010 a tabela diz Escorpião e o Sol está a 209,98°
// (Libra); 20/01/1988 diz Aquário e o Sol está a 299,64° (Capricórnio).
// Dizer o signo errado é o pior erro possível num app de astrologia, e ele caía
// justamente em quem nasceu na fronteira — a pessoa que MAIS presta atenção nisso.
//
// `timeStr` e `zona` são opcionais e usam a mesma convenção de moonSign e
// ascendantSign. Com eles, quem nasceu na virada recebe o signo certo pela
// primeira vez; sem eles, o meio-dia é o palpite que minimiza o erro.
export function signoFromDate(dateStr, timeStr, zona) {
  if (!dateStr) return null;
  if (!parseStrictDate(dateStr)) return null;

  const A = getAstronomy();
  if (A) {
    // Meio-dia como padrão: sem hora, é o instante que menos erra dentro do dia
    // (o Sol anda ~1°/dia, então a metade do dia é o ponto de menor distância
    // até qualquer hora real de nascimento).
    const ms = instanteUtcMs(dateStr, timeStr || "12:00", zona);
    if (!Number.isNaN(ms)) {
      try {
        // Longitude ECLÍPTICA APARENTE da data — o mesmo referencial trópico
        // que moonSign e ascendantSign já usam. Sem isso o app teria duas
        // definições de signo brigando dentro de si.
        const sol = A.Ecliptic(A.GeoVector(A.Body.Sun, new Date(ms), true));
        const lon = ((sol.elon % 360) + 360) % 360;
        return SIGNS[Math.floor(lon / 30)].name;
      } catch {
        // cai na tabela abaixo
      }
    }
  }

  // Reserva: só roda se o astronomy-engine não carregar (o require é protegido
  // por try/catch pro Metro não quebrar).
  //
  // AQUI HAVIA UMA TABELA DE DATAS FIXAS ("21/03 a 19/04 é Áries"). Medida
  // contra a efeméride real em 22.280 datas de 1950 a 2010, ela errava 248 —
  // 1,11%, um a cada noventa. E o erro não era aleatório: as cúspides do
  // calendário andam quase um dia a cada quatro anos por causa dos bissextos,
  // então a tabela chegava SEMPRE atrasada e devolvia o signo seguinte a quem
  // nasceu na virada. Escorpião->Libra sozinho eram 39 datas. Errar o signo
  // solar de alguém não é um detalhe: é errar a única coisa que a pessoa já
  // sabia antes de abrir o app, e é o eixo de todo o resto que ela vai ler.
  //
  // No lugar, a longitude eclíptica do Sol pela fórmula de baixa precisão do
  // Astronomical Almanac (USNO, seção C) — quatro linhas, erro abaixo de 0,02°
  // entre 1950 e 2050. Como 0,02° são cerca de 30 minutos de movimento solar e
  // a leitura é feita ao meio-dia, o resultado bate com o motor real em todas
  // as 22.280 datas medidas. Não é "fabricar céu": é a mesma grandeza que o
  // motor calcula, com menos casas decimais e a mesma definição trópica.
  const d = parseStrictDate(dateStr);
  const diasJ2000 = (Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), 12) - Date.UTC(2000, 0, 1, 12)) / 86400000;
  const rad = Math.PI / 180;
  const g = (357.528 + 0.9856003 * diasJ2000) * rad;          // anomalia média
  const L = 280.46 + 0.9856474 * diasJ2000;                   // longitude média
  const lon = L + 1.915 * Math.sin(g) + 0.02 * Math.sin(2 * g); // eclíptica aparente
  return SIGNS[Math.floor((((lon % 360) + 360) % 360) / 30)].name;
}

// ---------------------------------------------------------------------------
// COMPATIBILIDADE — agora derivada do ASPECTO, em lib/synastry.js
// ---------------------------------------------------------------------------
// O QUE HAVIA AQUI, E POR QUE SAIU (medido nesta base em 31/07/2026):
//
// Duas tabelas de 10 entradas (`PAIRS` e `PCT`) chaveadas pelo PAR DE
// ELEMENTOS. Rodando as 144 combinações de signo contra a distância zodiacal:
//
//   distância        relação             nota antiga     nota nova
//   ------------------------------------------------------------------
//   0  (12 pares)    mesmo signo           87–90        Co-presença
//   1  (24 pares)    aversão 30°           74–80        Aversão
//   2  (24 pares)    sextil                91–92        Sextil
//   3  (24 pares)    quadratura            74–80        Quadratura
//   4  (24 pares)    trígono               87–90        Trígono
//   5  (24 pares)    aversão 150°          74–80        Aversão
//   6  (12 pares)    OPOSIÇÃO              91–92        Oposição
//
// Menor 74, maior 92, média 83,5, ZERO pares abaixo de 70 — "todo mundo
// combina", que foi exatamente o que o dono percebeu sozinho. Pior: a OPOSIÇÃO,
// o aspecto mais duro da tradição, empatava na FAIXA MAIS ALTA (Áries+Libra =
// 92, a nota máxima do app inteiro), e a AVERSÃO — o único "não há relação" que
// Ptolomeu registra — recebia a mesma faixa da quadratura. Sete relações
// distintas viravam três faixas, com a ordem invertida no topo.
//
// A conta agora é a distância em signos (0–6), que é a doutrina de signo
// inteiro de Tetrabiblos I.13/I.16 — a mesma que lib/dailyThought.js já usa.
// Toda a leitura, as citações e a escala ordinal de IV.7 vivem em
// lib/synastry.js, com a pesquisa e as ressalvas escritas lá.
//
// A PORCENTAGEM ACABOU. `compatPercent()` não existe mais — não foi renomeada,
// foi removida. O motivo está inteiro no cabeçalho de lib/synastry.js
// ("A DECISÃO DA PORCENTAGEM"); o resumo é que o próprio app já proíbe
// porcentagem de compatibilidade no prompt da persona Luna, e a tela contradizia
// o prompt. Quem precisa exibir o resultado usa `.aspecto` e `.categoria`.
import { sinastria, nomeDoSigno } from "./synastry";

// `lang` é opcional e cai no PT quando ausente — chamada antiga continua
// devolvendo exatamente o texto de sempre (golden em test/golden/). A tela
// passa o idioma que já tem do useLanguage(); os packs vivem em
// lib/traducoes/synastry.<lang>.js.
export function compatibility(nameA, nameB, lang) {
  const ia = SIGNS.findIndex((s) => s.name === nameA);
  const ib = SIGNS.findIndex((s) => s.name === nameB);
  if (ia < 0 || ib < 0) return null;
  const a = SIGNS[ia];
  const b = SIGNS[ib];
  // O índice é o que carrega a geometria — sem ele não há distância, e sem
  // distância não há aspecto. É por isso que ele viaja junto do signo.
  const leitura = sinastria({ ...a, index: ia }, { ...b, index: ib }, lang);
  if (!leitura) return null;
  return {
    // O título também traduz (01/08/2026): antes vinha de SIGNS, que é
    // sempre português, e coroava um corpo inteiro em inglês —
    // "♊ Gêmeos + Escorpião ♏" em cima de um texto que dizia Gemini.
    titulo: `${a.emoji} ${nomeDoSigno(a.name, lang)} + ${nomeDoSigno(b.name, lang)} ${b.emoji}`,
    emojiA: a.emoji,
    emojiB: b.emoji,
    ...leitura,
  };
}

// Números "cósmicos" do casal — só por diversão, determinístico pela semente (nomes/signos).
export function cosmicNumbers(seed, count = 3) {
  count = Math.min(count, 99);
  let h = 0;
  for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) >>> 0;
  const out = [];
  while (out.length < count) {
    h = (h * 1103515245 + 12345) >>> 0;
    const num = (h % 99) + 1;
    if (!out.includes(num)) out.push(num);
  }
  return out;
}

const FREQUENCIAS = [
  "528Hz · frequência do amor",
  "639Hz · conexão e relacionamentos",
  "432Hz · harmonia",
  "741Hz · expressão",
];
export function frequenciaFor(seed) {
  let h = 0;
  for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) >>> 0;
  return FREQUENCIAS[h % FREQUENCIAS.length];
}

// Cartas do casal — mecânica "escolham 3 cartas", com significados positivos (entretenimento).
export const CARDS = [
  { name: "O Coração", emoji: "❤️", meaning: "amor e conexão sincera" },
  { name: "A Estrela", emoji: "✨", meaning: "esperança e sonhos que vocês compartilham" },
  { name: "A Viagem", emoji: "🧭", meaning: "aventuras esperando por vocês" },
  { name: "O Sol", emoji: "☀️", meaning: "alegria e dias luminosos juntos" },
  { name: "A Lua", emoji: "🌙", meaning: "intuição e intimidade" },
  { name: "O Laço", emoji: "🎀", meaning: "compromisso e cumplicidade" },
  { name: "A Chave", emoji: "🗝️", meaning: "um novo capítulo se abrindo" },
  { name: "O Brinde", emoji: "🥂", meaning: "celebração e momentos a dois" },
  { name: "O Lar", emoji: "🏡", meaning: "construir um lar juntos" },
];

// Lua — cálculo real de posição astronômica via astronomy-engine (pure JS, sem código nativo).
// Guardado em try/catch: se o pacote não estiver instalado no app RN, moonSign
// simplesmente retorna null em vez de quebrar o require/Metro bundling.
let _Astronomy = null;
function getAstronomy() {
  if (_Astronomy) return _Astronomy;
  try {
    _Astronomy = require("astronomy-engine");
  } catch {
    _Astronomy = false;
  }
  return _Astronomy;
}

// `zona` (3º argumento, OPCIONAL) é o fuso do lugar de nascimento — número,
// string IANA ou o objeto de cidade inteiro, o mesmo que ascendantSign aceita.
// Com ele, a hora informada é lida como hora de PAREDE e convertida no instante
// UTC certo (com horário de verão). Sem ele, nada muda: a hora continua sendo
// lida como UTC, que é o que o céu-de-hoje do resto do app quer. Ver
// instanteUtcMs() para os números que motivaram o argumento.
export function moonSign(dateStr, timeStr, zona) {
  if (!dateStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  const A = getAstronomy();
  if (!A) return null;
  const ms = instanteUtcMs(dateStr, timeStr, zona);
  if (Number.isNaN(ms)) return null;
  const d = new Date(ms);
  const eclip = A.EclipticGeoMoon(d);
  const lon = ((eclip.lon % 360) + 360) % 360;
  const idx = Math.floor(lon / 30);
  return SIGNS[idx];
}

// Ascendente — cálculo real via astronomy-engine (Astronomy.SiderealTime +
// Astronomy.e_tilt), fórmula clássica de Duffett-Smith expressa com atan2 pra
// não precisar de correção manual de quadrante:
//   RAMC = ((GAST + longitude/15) * 15) mod 360      // longitude: Leste = +, Oeste = −
//   y    = cos(RAMC)
//   x    = -(sin(eps) * tan(lat) + cos(eps) * sin(RAMC))
//   Asc  = atan2(y, x) normalizado pra [0, 360)
// (Sinal de y/x conferido empiricamente: no instante exato do nascer do Sol,
// o Ascendente tem que coincidir com a longitude eclíptica do próprio Sol —
// validado em vários pares lat/long com astronomy-engine antes de fechar o sinal;
// a forma "espelhada" ingênua desse mesmo formulário devolve o Descendente, 180° errado.)
//
// Ao contrário de moonSign (que aceita hora ausente e usa meio-dia como
// aproximação aceitável pro sinal da Lua), o Ascendente muda de signo quase a
// cada 2 horas — uma hora ausente ou "chutada" é quase cara-ou-coroa entre
// signos. Por isso esta função exige hora E localização reais: retorna null
// (nunca um valor fabricado) se faltar timeStr, latitude ou longitude.
//
// utcOffsetHours é o fuso padrão do local de nascimento (ex.: -3 pro horário
// de Brasília) usado pra converter a hora local informada em UTC antes do
// cálculo — sem isso, hora local seria tratada como se já fosse UTC (mesma
// lacuna que moonSign tem hoje, preservada ali para não mudar seu
// comportamento existente).
export function ascendantSign(dateStr, timeStr, latitude, longitude, utcOffsetHours) {
  if (!dateStr || !timeStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  if (typeof latitude !== "number" || typeof longitude !== "number" || Number.isNaN(latitude) || Number.isNaN(longitude)) {
    return null;
  }
  const A = getAstronomy();
  if (!A) return null;

  // Número → ele mesmo (comportamento antigo). String IANA / objeto de cidade
  // → offset REAL daquele instante, com horário de verão. Ver offsetHorasDe().
  const offset = offsetHorasDe(utcOffsetHours, dateStr, timeStr);
  // Trata "YYYY-MM-DDTHH:MM:00Z" como se os campos informados já fossem UTC,
  // só pra fazer a aritmética de data — depois subtrai o offset do fuso local
  // pra chegar no instante UTC real (ex.: 21:00 local em UTC-3 → 00:00Z do dia seguinte).
  const localAsUtcMs = new Date(`${dateStr}T${timeStr}:00Z`).getTime();
  if (Number.isNaN(localAsUtcMs)) return null;
  const utcMs = localAsUtcMs - offset * 3600 * 1000;
  const d = new Date(utcMs);

  const gastHours = A.SiderealTime(d); // Greenwich Apparent Sidereal Time, em horas siderais
  const tilt = A.e_tilt(A.MakeTime(d));
  const epsRad = (tilt.tobl * Math.PI) / 180; // obliquidade verdadeira da eclíptica
  const latRad = (latitude * Math.PI) / 180;

  const ramcDeg = (((gastHours + longitude / 15) * 15) % 360 + 360) % 360;
  const ramcRad = (ramcDeg * Math.PI) / 180;

  const y = Math.cos(ramcRad);
  const x = -(Math.sin(epsRad) * Math.tan(latRad) + Math.cos(epsRad) * Math.sin(ramcRad));
  const ascDeg = ((Math.atan2(y, x) * 180) / Math.PI % 360 + 360) % 360;

  const idx = Math.floor(ascDeg / 30);
  return SIGNS[idx];
}

// ---------------------------------------------------------------------------
// Mapa Astral avançado: Casas, Aspectos e Astrocartografia (v1 escopada).
// Tudo abaixo é ADIÇÃO pura — nenhuma linha das funções acima foi tocada.
// ---------------------------------------------------------------------------

// Grau exato do Ascendente (0–360), para uso interno das Casas e da
// astrocartografia. É a MESMA matemática de ascendantSign (RAMC/atan2,
// Duffett-Smith), duplicada aqui de propósito em vez de extraída para dentro
// de ascendantSign — ascendantSign precisa continuar byte-a-byte idêntica
// (outras partes do app já dependem dela), então esta cópia interna é o jeito
// seguro de reaproveitar o grau exato sem mexer na função original.
function _ascendantDegree(dateStr, timeStr, latitude, longitude, utcOffsetHours) {
  if (!dateStr || !timeStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  if (typeof latitude !== "number" || typeof longitude !== "number" || Number.isNaN(latitude) || Number.isNaN(longitude)) {
    return null;
  }
  const A = getAstronomy();
  if (!A) return null;

  // Número → ele mesmo (comportamento antigo). String IANA / objeto de cidade
  // → offset REAL daquele instante, com horário de verão. Ver offsetHorasDe().
  const offset = offsetHorasDe(utcOffsetHours, dateStr, timeStr);
  const localAsUtcMs = new Date(`${dateStr}T${timeStr}:00Z`).getTime();
  if (Number.isNaN(localAsUtcMs)) return null;
  const utcMs = localAsUtcMs - offset * 3600 * 1000;
  const d = new Date(utcMs);

  const gastHours = A.SiderealTime(d);
  const tilt = A.e_tilt(A.MakeTime(d));
  const epsRad = (tilt.tobl * Math.PI) / 180;
  const latRad = (latitude * Math.PI) / 180;

  const ramcDeg = (((gastHours + longitude / 15) * 15) % 360 + 360) % 360;
  const ramcRad = (ramcDeg * Math.PI) / 180;

  const y = Math.cos(ramcRad);
  const x = -(Math.sin(epsRad) * Math.tan(latRad) + Math.cos(epsRad) * Math.sin(ramcRad));
  return ((Math.atan2(y, x) * 180) / Math.PI % 360 + 360) % 360;
}

// Casas — sistema de Casas Inteiras (Whole Sign): Casa 1 = signo do
// Ascendente, Casa 2 = próximo signo, e assim por diante. Escolhido em vez de
// Casas Iguais (precisaria expor ascDeg fora desta função só pra depois
// arredondar de novo — sem ganho real) e em vez de Placidus (matematicamente
// indefinido perto/acima do círculo polar, ~66,5° de latitude — risco real
// num app global). Mesmas exigências de entrada e mesmo padrão
// "nunca fabricar" de ascendantSign: sem data, hora, latitude ou longitude
// reais, devolve null em vez de chutar uma casa.
export function houses(dateStr, timeStr, latitude, longitude, utcOffsetHours) {
  const ascDeg = _ascendantDegree(dateStr, timeStr, latitude, longitude, utcOffsetHours);
  if (ascDeg === null) return null;
  const idx = Math.floor(ascDeg / 30);
  const out = [];
  for (let n = 1; n <= 12; n++) {
    out.push({ houseNumber: n, sign: SIGNS[(idx + n - 1) % 12] });
  }
  return out;
}

// Planetas clássicos, na mesma ordem em que serão exibidos/comparados.
// Sol e Lua reaproveitam as mesmas fontes já usadas por signoFromDate/moonSign
// (SunPosition/EclipticGeoMoon, ambas já em "eclíptica da data" — o mesmo
// referencial); os demais usam GeoVector + Ecliptic (também eclíptica
// verdadeira da data), pra todos os planetas ficarem no mesmo referencial e
// não haver deriva por precessão entre eles.
const PLANETS = [
  { name: "Sol", body: null },
  { name: "Lua", body: null },
  { name: "Mercúrio", body: "Mercury" },
  { name: "Vênus", body: "Venus" },
  { name: "Marte", body: "Mars" },
  { name: "Júpiter", body: "Jupiter" },
  { name: "Saturno", body: "Saturn" },
  { name: "Urano", body: "Uranus" },
  { name: "Netuno", body: "Neptune" },
  { name: "Plutão", body: "Pluto" },
];

// Posições planetárias — longitude eclíptica (0–360°) de cada planeta clássico
// no instante informado. Só exige dateStr (mesmo padrão de moonSign: sem
// timeStr, usa meio-dia como aproximação aceitável — planetas, exceto a Lua,
// se movem pouco em algumas horas, então essa aproximação já validada em
// moonSign vale aqui também). Nunca fabrica: sem astronomy-engine disponível
// ou data inválida, devolve null.
// `zona` (3º argumento, OPCIONAL): idem moonSign — sem ele o comportamento é
// byte a byte o de sempre.
export function planetPositions(dateStr, timeStr, zona) {
  if (!dateStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  const A = getAstronomy();
  if (!A) return null;
  const ms = instanteUtcMs(dateStr, timeStr, zona);
  if (Number.isNaN(ms)) return null;
  const d = new Date(ms);

  return PLANETS.map(({ name, body }) => {
    let elon;
    if (name === "Sol") {
      elon = A.SunPosition(d).elon;
    } else if (name === "Lua") {
      elon = A.EclipticGeoMoon(d).lon;
    } else {
      elon = A.Ecliptic(A.GeoVector(A.Body[body], d, true)).elon;
    }
    return { planet: name, longitude: ((elon % 360) + 360) % 360 };
  });
}

// Menor separação angular (0–180°) entre duas longitudes eclípticas.
function _angularSeparation(lonA, lonB) {
  const diff = Math.abs(lonA - lonB);
  return diff > 180 ? 360 - diff : diff;
}

// Tabela de aspectos maiores e seus orbes-padrão (consenso Wikipedia/
// Astrotheme/Astrolibrary): conjunção 0° (orbe 8°), sextil 60° (orbe 6°),
// quadratura 90° (orbe 8°), trígono 120° (orbe 8°), oposição 180° (orbe 8°).
const ASPECTS_TABLE = [
  { type: "Conjunção", angle: 0, orb: 8 },
  { type: "Sextil", angle: 60, orb: 6 },
  { type: "Quadratura", angle: 90, orb: 8 },
  { type: "Trígono", angle: 120, orb: 8 },
  { type: "Oposição", angle: 180, orb: 8 },
];

// Aspectos — todas as combinações de pares entre os planetas clássicos,
// classificadas contra a tabela de orbes-padrão acima. Reaproveita
// planetPositions (mesma data/hora, mesmo "nunca fabricar": sem posições
// calculáveis, devolve null). orb aqui é o desvio real em relação ao ângulo
// exato do aspecto (quanto mais perto de 0, mais "exato" é o aspecto).
export function aspects(dateStr, timeStr, zona) {
  const positions = planetPositions(dateStr, timeStr, zona);
  if (!positions) return null;

  const out = [];
  for (let i = 0; i < positions.length; i++) {
    for (let j = i + 1; j < positions.length; j++) {
      const a = positions[i];
      const b = positions[j];
      const sep = _angularSeparation(a.longitude, b.longitude);
      for (const { type, angle, orb } of ASPECTS_TABLE) {
        const delta = Math.abs(sep - angle);
        if (delta <= orb) {
          out.push({
            planetA: a.planet,
            planetB: b.planet,
            aspectType: type,
            exactAngle: sep,
            orb: delta,
          });
          break; // tabela em ordem crescente de ângulo — primeiro encaixe é o aspecto mais próximo, evita duplicar o mesmo par em dois aspectos
        }
      }
    }
  }
  return out;
}

// Mercúrio retrógrado — calculado de verdade (nunca fabricado): compara a
// longitude eclíptica geocêntrica de Mercúrio dois dias antes e dois dias
// depois da data pedida; se ela caiu (considerando o giro de 360°), é
// porque o planeta está em movimento aparente retrógrado nessa janela —
// mesma definição usada por qualquer efeméride. Validado contra os 3
// períodos retrógrados reais de 2024 (fonte: CHANI/Hermetikon Mercury
// Retrograde 2024) em test/signs.test.js. Sem astronomy-engine ou data
// inválida, devolve null — nunca "sim"/"não" chutado.
export function isMercuryRetrograde(dateStr) {
  if (!dateStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  const A = getAstronomy();
  if (!A) return null;

  const center = new Date(`${dateStr}T12:00:00Z`);
  const before = new Date(center.getTime() - 2 * 24 * 60 * 60 * 1000);
  const after = new Date(center.getTime() + 2 * 24 * 60 * 60 * 1000);

  const lonBefore = A.Ecliptic(A.GeoVector(A.Body.Mercury, before, true)).elon;
  const lonAfter = A.Ecliptic(A.GeoVector(A.Body.Mercury, after, true)).elon;

  // Normaliza a diferença pra [-180, 180] — sem isso, uma virada de 359°
  // pra 1° pareceria um recuo gigante (-358°) em vez do avanço real (+2°).
  let diff = lonAfter - lonBefore;
  diff = ((diff + 540) % 360) - 180;

  return diff < 0;
}

// ---------------------------------------------------------------------------
// Astrocartografia — v1 escopada e textual (NÃO é um mapa interativo).
//
// O pacote não tem nenhuma lib de mapa (sem react-native-maps/expo-maps/
// mapbox no package.json) e um mapa real precisaria projetar curvas AC/DC/MC/IC
// não-lineares globo afora — cálculo bem mais arriscado e sem nenhum caso de
// referência pra validar. Em vez disso, esta v1 reaproveita a MESMA matemática
// de RAMC/atan2 já validada em ascendantSign, aplicada a uma lista curada de
// cidades notáveis (latitude/longitude conhecidas, todas abaixo do círculo
// polar) para o instante exato de nascimento, e reporta em quais cidades cada
// planeta cai perto de um dos 4 ângulos (Ascendente/Descendente/Meio-do-Céu/
// Fundo do Céu) — texto tipo "Marte perto do Meio-do-Céu em Tóquio", nunca
// fabricado, sempre calculado por cidade real.
const NOTABLE_CITIES = [
  { name: "São Paulo", lat: -23.5505, lon: -46.6333 },
  { name: "Rio de Janeiro", lat: -22.9068, lon: -43.1729 },
  { name: "Buenos Aires", lat: -34.6037, lon: -58.3816 },
  { name: "Cidade do México", lat: 19.4326, lon: -99.1332 },
  { name: "Nova York", lat: 40.7128, lon: -74.006 },
  { name: "Los Angeles", lat: 34.0522, lon: -118.2437 },
  { name: "Londres", lat: 51.5074, lon: -0.1278 },
  { name: "Lisboa", lat: 38.7223, lon: -9.1393 },
  { name: "Paris", lat: 48.8566, lon: 2.3522 },
  { name: "Roma", lat: 41.9028, lon: 12.4964 },
  { name: "Cairo", lat: 30.0444, lon: 31.2357 },
  { name: "Moscou", lat: 55.7558, lon: 37.6173 },
  { name: "Dubai", lat: 25.2048, lon: 55.2708 },
  { name: "Nova Délhi", lat: 28.6139, lon: 77.209 },
  { name: "Bangcoc", lat: 13.7563, lon: 100.5018 },
  { name: "Pequim", lat: 39.9042, lon: 116.4074 },
  { name: "Tóquio", lat: 35.6762, lon: 139.6503 },
  { name: "Sydney", lat: -33.8688, lon: 151.2093 },
  { name: "Joanesburgo", lat: -26.2041, lon: 28.0473 },
  { name: "Reykjavik", lat: 64.1466, lon: -21.9426 },
];

const ANGLE_ORB = 3; // graus de tolerância pra considerar um planeta "angular"

// Ascendente/Meio-do-Céu (e seus opostos) para UMA cidade candidata, no
// instante UTC exato de nascimento — mesma fórmula de ascendantSign, só
// trocando a latitude/longitude de nascimento pela da cidade candidata.
// Meio-do-Céu aqui é aproximado pelo próprio RAMC (sem depender de latitude,
// como o relatório recomenda) — aproximação suficiente pra uma lista textual
// de cidades notáveis, não para um mapa de precisão.
function _cityAngles(utcMs, cityLat, cityLon, A) {
  const d = new Date(utcMs);
  const gastHours = A.SiderealTime(d);
  const tilt = A.e_tilt(A.MakeTime(d));
  const epsRad = (tilt.tobl * Math.PI) / 180;
  const latRad = (cityLat * Math.PI) / 180;

  const ramcDeg = (((gastHours + cityLon / 15) * 15) % 360 + 360) % 360;
  const ramcRad = (ramcDeg * Math.PI) / 180;

  const y = Math.cos(ramcRad);
  const x = -(Math.sin(epsRad) * Math.tan(latRad) + Math.cos(epsRad) * Math.sin(ramcRad));
  const ascDeg = ((Math.atan2(y, x) * 180) / Math.PI % 360 + 360) % 360;
  const dscDeg = (ascDeg + 180) % 360;
  const mcDeg = ramcDeg;
  const icDeg = (ramcDeg + 180) % 360;

  return [
    { point: "Ascendente", degree: ascDeg },
    { point: "Descendente", degree: dscDeg },
    { point: "Meio-do-Céu", degree: mcDeg },
    { point: "Fundo do Céu", degree: icDeg },
  ];
}

// Astrocartografia v1: para o instante exato de nascimento (dateStr + timeStr
// + utcOffsetHours da cidade de nascimento, mesma conversão pra UTC real que
// ascendantSign já faz), varre a lista curada de cidades notáveis e devolve
// só as combinações planeta+cidade+ângulo que caem dentro do orbe de
// tolerância. Exige dateStr e timeStr reais (nunca fabrica hora); sem eles,
// ou sem astronomy-engine disponível, devolve null.
export function astrocartographyCities(dateStr, timeStr, utcOffsetHours) {
  if (!dateStr || !timeStr) return null;
  if (!parseStrictDate(dateStr)) return null;
  const A = getAstronomy();
  if (!A) return null;

  // BUG CORRIGIDO EM 30/07/2026: esta linha era `planetPositions(dateStr,
  // timeStr)`, SEM o fuso — os ângulos de cada cidade eram calculados no
  // instante UTC certo (utcMs, logo abaixo) e os planetas num instante 2 a 3
  // horas diferente, dentro da MESMA função. Como ANGLE_ORB é 3°, e a Lua anda
  // até 1,9° em 3h em São Paulo (até 8,3° em Auckland), a deriva era da ordem
  // do próprio orbe: linhas planetárias apareciam e sumiam por erro de conta.
  const positions = planetPositions(dateStr, timeStr, utcOffsetHours);
  if (!positions) return null;

  // Número → ele mesmo (comportamento antigo). String IANA / objeto de cidade
  // → offset REAL daquele instante, com horário de verão. Ver offsetHorasDe().
  const offset = offsetHorasDe(utcOffsetHours, dateStr, timeStr);
  const localAsUtcMs = new Date(`${dateStr}T${timeStr}:00Z`).getTime();
  if (Number.isNaN(localAsUtcMs)) return null;
  const utcMs = localAsUtcMs - offset * 3600 * 1000;

  const out = [];
  for (const city of NOTABLE_CITIES) {
    const angles = _cityAngles(utcMs, city.lat, city.lon, A);
    for (const planet of positions) {
      for (const { point, degree } of angles) {
        const sep = _angularSeparation(planet.longitude, degree);
        if (sep <= ANGLE_ORB) {
          out.push({ city: city.name, planet: planet.planet, point, orb: sep });
        }
      }
    }
  }
  return out;
}

// POLARIDADE E MODALIDADE (11/09/2026) — os dois atributos fixos do signo que
// faltavam na ficha (o elemento já vinha em SIGNS[].element). Não é leitura:
// é a tabela clássica, e ela é ARITMÉTICA sobre a ordem zodiacal — Áries é o
// índice 0, Peixes o 11:
//   polaridade: par = masculino (Áries, Gêmeos, Leão, Libra, Sagitário,
//               Aquário), ímpar = feminino;
//   modalidade: índice % 3 → 0 cardinal, 1 fixo, 2 mutável.
// Nome desconhecido → null, e a tela não mostra nada (nunca um chute).
const MODALIDADES = ['cardinal', 'fixo', 'mutavel'];
function indiceDoSigno(name) {
  const i = SIGNS.findIndex((s) => s.name === name);
  return i === -1 ? null : i;
}
// O elemento pelo NOME canônico — displaySign() (BirthChartScreen) devolve só
// o que a tela mostra (label, cor, glifo) e não carrega `element`; quem só tem
// o nome do signo consulta aqui. Mesmo contrato dos dois de baixo: null se
// o nome não existir.
export function elementoDoSigno(name) {
  const i = indiceDoSigno(name);
  return i === null ? null : SIGNS[i].element;
}
export function polaridadeDoSigno(name) {
  const i = indiceDoSigno(name);
  return i === null ? null : (i % 2 === 0 ? 'masculino' : 'feminino');
}
export function modalidadeDoSigno(name) {
  const i = indiceDoSigno(name);
  return i === null ? null : MODALIDADES[i % 3];
}
