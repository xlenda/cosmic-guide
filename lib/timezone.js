...
// 1985-2019. ATENÇÃO ao mapa de quem participou, porque a intuição erra
// (conferido em 30/07/2026 varrendo mês a mês, de 1900 a 2025, cada fuso IANA
// do Brasil com a própria offsetHoursFor() deste arquivo):
//
//   · 1931-33, 1949-53, 1963-68 e 1985-88 → o país INTEIRO, inclusive Manaus,
//     Belém, Boa Vista, Porto Velho, Rio Branco e Fernando de Noronha.
//   · 1989 em diante → só Sul/Sudeste/Centro-Oeste, com idas e vindas do
//     Nordeste (Recife/Fortaleza/Noronha 1989-90 e 1999-2002; Maceió e
//     Araguaína também em 1995-96; Bahia até 2003 e de novo em 2011-2012;
//     Amazonas voltou em 1993-94; Acre em 2008 e 2013).
//
// Ou seja, "no Norte/Nordeste não tinha horário de verão" é falso pra qualquer
// nascimento anterior a 1989. Não dá pra resolver isso com tabela na mão.
// Indo pro mundo (EUA e Europa inteiros com DST ATIVO até hoje) deixa de ser
// caso de borda e vira o caso comum.
//
// LIMITE HONESTO DA TZDATA: o próprio projeto IANA só garante os dados
// pré-1970 para a cidade PRINCIPAL de cada fuso. "America/Chicago" está certo
// para Chicago em 1946; para o Arkansas de 1946, que não seguiu a mesma regra
// local, pode não estar. Isso não tem conserto no cliente e não é motivo pra
// voltar ao offset fixo — o fixo erra SEMPRE, a tzdata erra num punhado de
// casos pré-1970 fora da cidade de referência.
//
...
//    o offset de novo no ponto corrigido. Duas passadas bastam para toda
//    transição de horário de verão (30min ou 1h) e para toda mudança de fuso
//    normal. NÃO bastam para os dois pulos de LINHA DE DATA que existem na
//    tzdata — Pacific/Kiritimati em 31/12/1994 (+14 ↔ -10, o dia inteiro
//    deixou de existir) e Pacific/Apia em 30/12/2011. Nesses dois dias o
//    resultado é um dos dois offsets válidos, escolhido em silêncio; são datas
//    que localmente NUNCA existiram, então não há resposta certa a dar.
...
//    calculado offline e outro online. CONFERIDO em 30/07/2026: as duas
//    implementações foram executadas lado a lado em 144.480 combinações
//    fuso×data×hora, cobrindo os 40 fusos da reserva offline de 1940 a 2025 —
//    ZERO divergências.)
//
...
//        2015-03-08 02:30 em Nova York → -4).
//      · hora ambígua      → a PRIMEIRA ocorrência, a que ainda está no
//        horário de verão (2015-02-21 23:30 em São Paulo → -2;
//        2015-11-01 01:30 em Nova York → -4).
//    RESSALVA conferida em 30/07/2026: a regra da hora inexistente vale para
//    saltos de 1 hora, que são todos os do mundo menos um. Na Ilha Lord Howe
//    (Austrália), único lugar com salto de 30 minutos, a hora inexistente cai
//    no offset ANTERIOR (2015-10-04 02:15 → +10:30, não +11). São 382
//    habitantes e 30 minutos, então não vale um terceiro passo no algoritmo —
//    mas fica escrito, porque documentação que promete o que o código não faz
//    é pior que documentação nenhuma.
...
/**
 * O `utcOffsetAt` carimbado (`utcOffsetAtFor`) é do MESMO nascimento que está
 * sendo calculado agora? Formato do carimbo: "YYYY-MM-DD" ou "YYYY-MM-DDTHH:mm",
 * exatamente o que components/CityPickerModal.js monta e manda como `at`.
 *
 * Carimbo AUSENTE devolve false de propósito: ou a cidade foi salva por uma
 * versão anterior a este carimbo (e aí não dá pra saber de que data ela é), ou
 * veio sem `at` nenhum. Nos dois casos o `utcOffset` fixo é a escolha honesta.
 */
function atCombina(carimbo, dateStr, timeStr) {
  if (typeof carimbo !== 'string' || !carimbo) return false;
  if (typeof dateStr !== 'string' || !dateStr) return false;
  const esperado = timeStr ? `${dateStr}T${timeStr}` : dateStr;
  return carimbo === esperado;
}
...
export function resolveOffsetHours(zona, dateStr, timeStr) {
  ...
  if (zona && typeof zona === 'object') {
    ...
    //
    // E só se ele for DESTE nascimento. O campo é uma fotografia tirada no
    // momento em que a cidade foi escolhida, e ela é salva junto com a cidade:
    // quem escolheu a cidade com "20/01/2015" e depois corrigiu pra
    // "20/07/2015" tem guardado um -2 que virou mentira (o certo passou a ser
    // -3, que é justamente o `utcOffset` fixo logo abaixo). `utcOffsetAtFor`,
    // carimbado por lib/cities.js, é o `at` que gerou o número — se não bate
    // com a data/hora de agora, o campo é descartado e caímos no fixo, que
    // pelo menos nunca é de OUTRO nascimento.
    if (
      typeof zona.utcOffsetAt === 'number' &&
      !Number.isNaN(zona.utcOffsetAt) &&
      atCombina(zona.utcOffsetAtFor, dateStr, timeStr)
    ) {
