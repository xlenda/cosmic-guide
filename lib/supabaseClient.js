...
// No Android QUALQUER app instalado no aparelho consegue disparar
// 'cosmicguide://?code=lixo'. A troca falha (o código é inválido), mas o SDK
// APAGA o code verifier do PKCE nos dois caminhos — sucesso e catch, ver
// GoTrueClient._exchangeCodeForSession — então um app malicioso conseguia
// impedir a vítima de entrar. O jeito certo seria conferir o `state` do OAuth,
// mas o supabase-js não tem esse parâmetro: quem cuida do state é o servidor
// do Supabase, entre ele e o Google, e o que volta pro app é só '?code='. A
// marca de "fui EU que pedi este login" tem que ser nossa mesmo. Precisa
// sobreviver ao restart: o link de e-mail é tocado com o app fechado.
const MARCA_LOGIN = 'cosmicguide.login.emCurso';
// Mesma validade máxima que o Supabase dá aos links de e-mail (24h): passou
// disso, o código já não valeria de qualquer jeito.
// ponytail: marca única, não por código — o Supabase não devolve nada nosso
// junto do code pra casar um com o outro. Enquanto um login legítimo está em
// curso, a janela continua aberta; fechá-la de vez exigiria um nonce dentro do
// redirectTo (e isso depende de reconfigurar a lista de Redirect URLs do
// projeto Supabase à mão).
const MARCA_VALIDADE_MS = 24 * 60 * 60 * 1000;

async function marcarLoginEmCurso() {
  try {
    await AsyncStorage.setItem(MARCA_LOGIN, String(Date.now() + MARCA_VALIDADE_MS));
  } catch {}
}

async function limparLoginEmCurso() {
  try {
    await AsyncStorage.removeItem(MARCA_LOGIN);
  } catch {}
}

async function efetuarTroca(code) {
  // O portão fica AQUI, e não em trocarCodigoPorSessao, porque lá o
  // trocas.set() precisa continuar SÍNCRONO: é ele que garante uma troca só
  // quando a mesma emissão chega pelos dois ouvintes do Android. Um await
  // antes dele deixaria os dois caminhos passarem pelo Map vazio.
  try {
    const ate = await AsyncStorage.getItem(MARCA_LOGIN);
    if (!(Number(ate) > Date.now())) return {};
  } catch {
    return {};
  }
  try {
    ...
    // Entrou: a porta fecha atrás dela.
    await limparLoginEmCurso();
...
async function trocarCodigoPorSessao(url) {
  ...
  // Este caminho NÃO limpa a marca de login em curso de propósito: quem manda
  // a URL pode ser qualquer app do aparelho, e apagar a marca aqui devolveria
  // ao atacante justamente o poder de travar o login da vítima.
...
export async function signUpWithEmail(email, password) {
  ...
  if (error) return { error: traduzErro(error.message) };
  // Sem sessão de volta = vai chegar um link de e-mail; é só nesse caso que o
  // deep link de volta é esperado (e a marca precisa esperar por ele).
  if (!data.session) await marcarLoginEmCurso();
...
export async function signInWithGoogle() {
  ...
  if (error) return { error: traduzErro(error.message) };
  if (!NATIVO) return {};
  // A partir daqui o deep link de volta é NOSSO — antes de abrir a aba, senão
  // o retorno pode chegar com a porta ainda fechada.
  await marcarLoginEmCurso();
  ...
  // Não vem deep link nenhum depois disso, então a porta fecha junto.
