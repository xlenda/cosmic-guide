...
// Packs de texto por idioma (mesmos arquivos usados por lib/calendarioCosmico.js).
// São dados puros — importá-los aqui não cria ciclo: calendarioCosmico importa
// este arquivo, este arquivo importa só os packs.
...
// resultado (mesmo tom honesto de lib/palmReadings.js e lib/orbiConversation.js:
...
// i18n (31/07/2026): `key` indexa o pack de texto por idioma
// (lib/traducoes/calendario.*.js, bloco `fases`). Os campos `name`/`reflexao`
// daqui são o PT canônico e NÃO mudam de byte — outros módulos casam pelo
// `name` (lib/rituais.js exige byte a byte, lib/wallpaper.js idem) e o
// caminho sem `lang` continua devolvendo exatamente estes valores.
const PHASES = [
  {
    key: "luaNova",
    name: "Lua Nova",
...
// Nome e reflexão da fase no idioma pedido. 'pt' (e qualquer valor
// desconhecido) devolve os campos ORIGINAIS de PHASES — mesmas referências de
// string, byte a byte, que é o contrato de compatibilidade com todo consumidor
// antigo (rituais, wallpaper, cosmicSound, dailyHoroscope chamam sem lang).
function textosDaFase(phase, lang) {
  if (lang === "es" || lang === "en") {
    const t = PACKS[lang].fases[phase.key];
    if (t && t.nome && t.reflexao) return { name: t.nome, reflexao: t.reflexao };
  }
  return { name: phase.name, reflexao: phase.reflexao };
}
...
//
// `lang` ('pt' | 'es' | 'en', default e fallback 'pt') traduz SÓ os textos
// (name/reflexao); longitude, emoji e iluminação são medidos, não têm idioma.
export function getMoonPhase(date, lang = "pt") {
  ...
  const textos = textosDaFase(phase, lang);
  ...
  return {
    ...
    name: textos.name,
    emoji: phase.emoji,
    reflexao: textos.reflexao,
    illumination, // 0–100, ou null se o pacote não devolveu phase_fraction
  };
}

// ---------------------------------------------------------------------------
// A ÂNCORA ÚNICA DO DIA — meio-dia LOCAL
// ---------------------------------------------------------------------------
// Até 03/08/2026 o app tinha TRÊS âncoras diferentes pra "a fase da Lua de
// hoje", e nenhuma delas sabia da outra:
//
//   meio-dia UTC  — dailyHoroscope, dailyThought, cosmicSound (`T12:00:00Z`)
//   agora         — getMoonPhaseToday e rituaisDeHoje (`new Date()`)
//   meio-dia local— a grade do calendário (getMoonPhaseForMonth)
//
// O rótulo de fase vem de uma fatia de 45° (~3,7 dias), então perto de uma
// borda as três respondiam coisas diferentes pro MESMO dia: a grade do
// calendário marcava "Lua Cheia" na casinha de hoje enquanto o card logo
// acima dizia "Gibosa Minguante", e um ritual que pede Lua Cheia sumia da
// lista no mesmo instante. Um app que se contradiz sozinho na mesma tela é
// pior do que um que erra junto.
//
// POR QUE MEIO-DIA LOCAL e não meio-dia UTC: "a fase do dia 3" é um rótulo de
// DIA DE CALENDÁRIO, e o calendário de quem lê é o local. Meio-dia UTC tem a
// vantagem de dar o mesmo rótulo no mundo inteiro, mas em troca desalinha do
// calendário na tela — e é justamente a grade que a pessoa olha.
//
// `getMoonPhase(instante)` continua existindo e continua certo para quem
// pergunta por um INSTANTE de verdade (calendarioCosmico lê a iluminação no
// milissegundo exato do marco lunar). Esta função é para quem pergunta por um
// DIA.
export function meioDiaLocal(data) {
  const d = data instanceof Date ? data : new Date(data);
  if (Number.isNaN(d.getTime())) return null;
  return new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12, 0, 0, 0);
}

// A fase de um DIA — de 'YYYY-MM-DD' ou de um Date qualquer, sempre ancorada
// no meio-dia local daquele dia.
export function faseDoDia(dia, lang = "pt") {
  const base =
    typeof dia === "string" && /^\d{4}-\d{2}-\d{2}/.test(dia)
      ? new Date(Number(dia.slice(0, 4)), Number(dia.slice(5, 7)) - 1, Number(dia.slice(8, 10)), 12, 0, 0, 0)
      : meioDiaLocal(dia);
  return base ? getMoonPhase(base, lang) : null;
}

// Atalho pra "hoje". Ancorado no meio-dia local do dia de hoje — a MESMA
// âncora da grade do calendário, que é o ponto inteiro desta seção.
export function getMoonPhaseToday(lang = "pt") {
  return faseDoDia(new Date(), lang);
}
...
export function getMoonPhaseForMonth(year, month, lang = "pt") {
  const daysInMonth = new Date(year, month, 0).getDate();
  const days = [];
  for (let day = 1; day <= daysInMonth; day++) {
    const d = new Date(year, month - 1, day, 12, 0, 0);
    days.push({ day, date: d, phase: getMoonPhase(d, lang) });
  }
  return days;
}
...
export function getMoonPhaseForCurrentMonth(lang = "pt") {
  const now = new Date();
  return getMoonPhaseForMonth(now.getFullYear(), now.getMonth() + 1, lang);
}
