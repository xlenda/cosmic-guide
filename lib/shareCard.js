...
// EXPORTADA (09/08/2026): a Home passou a EXIBIR no card da frase o mesmo
// fundo que o compartilhar desenha — relato real do dono: "aparece a imagem
// nova mas compartilha a antiga". Fonte única = exibido e compartilhado
// nunca mais divergem.
export async function fundoDoDia(tipo) {
...
// O CARD MOSTRA O SOCO, NAO O TRATADO (04/08/2026). O Pensamento do Dia tem
// ~1.100 caracteres e desenhado inteiro a 58px estourava a arte (achado de
// agente durante a inversao quente-primeiro). Pro card vale o primeiro
// paragrafo — que agora E o gancho de reflexao — cortado em fim de frase; o
// resto a pessoa le no app, que e exatamente pra onde o link aponta.
function encurtarParaCard(frase, teto = 300) {
  const primeiro = String(frase || '').split('\n\n')[0].trim();
  if (primeiro.length <= teto) return primeiro;
  const corte = primeiro.slice(0, teto);
  const fimDeFrase = Math.max(corte.lastIndexOf('. '), corte.lastIndexOf('! '), corte.lastIndexOf('? '));
  return fimDeFrase > 80 ? corte.slice(0, fimDeFrase + 1) : corte.trimEnd() + '…';
}

function desenharCard(ctx, img, fraseCompleta, marca) {
  const frase = encurtarParaCard(fraseCompleta);
