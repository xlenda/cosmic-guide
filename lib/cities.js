...
function sanearCidade(bruta) {
  ...
  // O `at` que gerou aquele utcOffsetAt. Ver o porquê em carimbarAt().
  if (typeof bruta.utcOffsetAtFor === 'string' && bruta.utcOffsetAtFor) {
    cidade.utcOffsetAtFor = bruta.utcOffsetAtFor;
  }
...
// ---------------------------------------------------------------------------
// `utcOffsetAt` é uma FOTOGRAFIA, e fotografia envelhece.
//
// O servidor só devolve utcOffsetAt quando a busca manda `at` — a data/hora de
// nascimento que a tela tinha NAQUELE momento. Só que a cidade escolhida é
// salva inteira no aparelho, com o utcOffsetAt junto, e a pessoa pode voltar
// depois e mudar só a data de nascimento sem reabrir o seletor de cidade.
//
// A partir daí o número guardado é de outro nascimento. No aparelho normal
// isso não aparece (lib/timezone.js calcula o offset local e nem olha pro
// campo), mas num Android sem tzdata completa o utcOffsetAt velho é justamente
// o que seria usado — e ele pode ser PIOR que o `utcOffset` fixo: cidade
// escolhida com data em janeiro/2015 (horário de verão, -2) e nascimento
// depois corrigido pra julho/2015 daria -2 onde o campo fixo daria -3, o certo.
//
// Carimbar o `at` que gerou o número resolve: quem for usar confere se ainda é
// o mesmo nascimento. Custo: uma string por cidade.
// ---------------------------------------------------------------------------
function carimbarAt(cidade, at) {
  if (!cidade) return cidade;
  if (typeof cidade.utcOffsetAt !== 'number' || !at) return cidade;
  return { ...cidade, utcOffsetAtFor: at };
}
...
export async function searchCitiesRemote(query, opts = {}) {
  ...
  const itens = Array.isArray(dados && dados.items)
    ? dados.items.map(sanearCidade).filter(Boolean).map((c) => carimbarAt(c, at))
    : [];
...
export async function fetchCityById(id, opts = {}) {
  ...
  const cidade = carimbarAt(sanearCidade(dados && dados.city), at);
