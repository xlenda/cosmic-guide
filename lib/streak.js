...
// A tela de Relatórios importa daqui, como já importa getMonthActivity — não
// precisa conhecer lib/streakDays.js.
export { getMonthTypes } from './streakDays';
...
//
// `tipo` (opcional, 11/09/2026) é o slug da atividade ('tarot', 'dream',
// 'grounding'...) que colore o dia no calendário dos Relatórios. Vai pro mapa
// PARALELO (lib/streakDays.js, ACTIVE_TYPES_KEY), nunca pro
// `cosmic-active-days` — e em try/catch próprio: é cosmético, então falha
// nele não pode derrubar o dia ativo já gravado nem o retorno desta função.
// Sem tipo (chamadas antigas), o comportamento é o mesmo de sempre.
export async function recordActiveDay(tipo) {
  ...
  if (typeof tipo === 'string' && tipo) {
    try {
      await recordActivityType(tipo, today);
    } catch {}
  }
