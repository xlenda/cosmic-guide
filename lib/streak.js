...
// Setado ('1') após a PRIMEIRA leitura de streak pós-correção UTC->local —
// enquanto ausente, computeCurrentStreak aplica a tolerância de migração de
// 1 dia (ver comentário lá) exatamente uma vez.
const UTC_MIGRATION_KEY = 'cosmic-active-days-local-migrated';
...
async function computeCurrentStreak(days) {
  ...
  // MIGRAÇÃO UTC->LOCAL (tolerância de 1 dia, uma única vez): as chaves
  // antigas foram gravadas em UTC, então quem usou o app ontem à noite
  // (>21h no Brasil) tem o registro gravado como HOJE e ontem-local vazio —
  // sem esta janela, a virada de convenção zeraria a sequência (ou gastaria
  // um Escudo comprado) de quem usou o app todos os dias. Na primeira
  // leitura pós-update, se ontem-local está vazio mas hoje-local tem registro
  // (a versão UTC-deslocada do uso de ontem), ontem conta como coberto e fica
  // gravado como 'migrated' (permanente, como o 'shield'). O flag é setado
  // já na primeira leitura, com ou sem buraco — a tolerância nunca "sobra"
  // pra perdoar uma falta real dali a uma semana.
  let migrationTolerance = false;
  try {
    migrationTolerance = !(await AsyncStorage.getItem(UTC_MIGRATION_KEY));
    if (migrationTolerance) await AsyncStorage.setItem(UTC_MIGRATION_KEY, '1');
  } catch {}
  const todayKey = todayStr();
  const yesterdayDate = new Date();
  yesterdayDate.setDate(yesterdayDate.getDate() - 1);
  const yesterdayKey = todayStr(yesterdayDate);
  ...
  for (;;) {
    ...
    if (migrationTolerance && key === yesterdayKey && days[todayKey]) {
      // Buraco de ontem coberto pela migração (ver comentário acima) — nunca
      // consome escudo pra isso, e nunca acontece de novo (flag já gravado).
      days[key] = 'migrated';
      mutated = true;
      streak += 1;
      cursor.setDate(cursor.getDate() - 1);
