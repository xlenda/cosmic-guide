...
export const VERBATIM = {
  ...
  duradouro: {
    ...
    parafrase:
      'Os casamentos costumam durar quando o Sol e a Lua das duas cartas estão em aspecto harmônico — trígono ou sextil —, sobretudo trocando de lugar, e mais ainda quando a Lua do marido faz esse aspecto com o Sol da esposa.',
...
const PLANETA = {
  Marte: {
    quimica: 'quer sem rodeio e gosta da conquista',
    conversa: 'puxa o que precisa ser resolvido hoje',
  ...
  'Vênus': {
    quimica: 'quer ser desejado com calma e capricho',
    conversa: 'puxa gosto, afeto e quem tratou quem de que jeito',
    briga: 'fecha a cara e adia a conversa',
    casa: 'quer a casa bonita e o clima leve',
    prazo: 'precisa se sentir escolhido de novo, e com frequência',
  },
  'Mercúrio': {
    quimica: 'esquenta pela conversa muito antes de esquentar pelo resto',
    conversa: 'puxa notícia, teoria e o assunto que apareceu ontem',
    briga: 'argumenta pra ganhar, não pra entender',
    casa: 'organiza, faz lista e distribui tarefa',
    prazo: 'precisa que ainda exista assunto novo entre os dois',
  },
  Lua: {
    quimica: 'quer proximidade, colo e cheiro conhecido',
    conversa: 'puxa o que sentiu e o que ficou atravessado',
    briga: 'se recolhe e faz silêncio pesado',
    casa: 'cuida do miúdo que ninguém vê',
    prazo: 'precisa se sentir em segurança pra continuar aberto',
  },
  Sol: {
    quimica: 'quer ser visto e escolhido na frente dos outros',
    conversa: 'puxa plano grande e o que estão construindo juntos',
    briga: 'se fere no orgulho e endurece',
    casa: 'quer ser a referência da casa',
    prazo: 'precisa ouvir o reconhecimento em voz alta',
  },
  'Júpiter': {
    quimica: 'quer aventura, riso e um pouco de exagero',
    conversa: 'puxa viagem, futuro e o sentido das coisas',
    briga: 'solta uma verdade grande demais e depois se arrepende',
    casa: 'gasta mais do que planejou e promete mais do que cabe',
    prazo: 'precisa de horizonte aberto, não de coleira',
  },
  Saturno: {
    quimica: 'demora a se soltar, e quando solta é pra valer',
    conversa: 'puxa trabalho, dinheiro e o que dá pra sustentar',
    briga: 'fica frio e passa a cobrar',
...
//     e Retório: predomina o lugar que está no signo ANTERIOR, isto é, aquele a
//     partir do qual o outro é o décimo signo. Confere: o décimo signo a partir
//     de Câncer é Áries, e é Áries que predomina.
...
function fraseQuemPuxa(c) {
  if (c.lider) return `quem costuma dar o primeiro passo é ${c.lider}, e quem cede — quando cede — é ${c.segue}`;
  if (c.distancia === 0) return 'ninguém puxa e ninguém cede, porque a vontade chega nos dois na mesma hora';
  return 'nenhum dos dois cede primeiro, porque nas duas pontas do mesmo eixo a força é igual e a direção é oposta';
}
...
//
//   • DOIS PLANETAS DIFERENTES (o caso comum): um ponto e vírgula separando o
//     que move cada um.
...
function fraseRegentes(c, campo, abertura) {
  const a = PLANETA[c.regA][campo];
  const b = PLANETA[c.regB][campo];
  if (c.A === c.B) {
    return `${abertura} sendo o mesmo signo dos dois lados, cada um ${a} — o traço vem em dobro e não sobra ninguém pra fazer contraponto.`;
  }
  if (c.regA === c.regB) {
    return `${abertura} ${c.A} e ${c.B} ${MESMO_REGENTE_LEAD[campo]}, porque na conta antiga os dois pertencem ao mesmo planeta, e cada um ${a}.`;
  }
  return `${abertura} ${c.A} ${a}; ${c.B} ${b}.`;
}
...
const QUIMICA_FIGURA = {
  copresenca: (c) => `${c.A} com ${c.B} é desejo de espelho: cada um reconhece a própria vontade no outro e acende sem precisar traduzir nada — ${fraseQuemPuxa(c)}.`,
  trigono: (c) => `Entre ${c.A} e ${c.B} o desejo corre solto porque os dois querem a mesma coisa e no mesmo idioma — ${fraseQuemPuxa(c)}.`,
  sextil: (c) => `Entre ${c.A} e ${c.B} a atração é de curiosidade: há um ponto de encontro de verdade e diferença suficiente pra manter graça — ${fraseQuemPuxa(c)}.`,
  quadratura: (c) => `Entre ${c.A} e ${c.B} o desejo nasce do atrito: o que irrita de dia é exatamente o que puxa de noite — ${fraseQuemPuxa(c)}.`,
  oposicao: (c) => `Entre ${c.A} e ${c.B} a atração funciona como imã: o que fascina em um é justamente o que falta no outro — ${fraseQuemPuxa(c)}.`,
  aversao30: (c) => `Entre ${c.A} e ${c.B} o desejo não vem pronto: falta aquela faísca de reconhecimento imediato, e o que existe foi construído no convívio — ${fraseQuemPuxa(c)}.`,
  aversao150: (c) => `Entre ${c.A} e ${c.B} a atração costuma nascer de fora pra dentro, porque nada no encontro dos dois é automático — ${fraseQuemPuxa(c)}.`,
};
...
const QUIMICA_ELEMENTO = {
  ...
  'fogo+água': (c) => `${c.el('fogo')} avança e ${c.el('água')} sente antes de responder — quando o tempo dos dois coincide é elétrico, e quando não coincide um se sente rejeitado e o outro invadido.`,
...
const QUIMICA_MODALIDADE = {
  'cardeal+cardeal': () => 'O que esfria: os dois começam e nenhum sustenta, então a rotina do desejo é sempre o que fica pra depois.',
  'fixo+fixo': () => 'O que esfria: quando o desejo vira hábito, os dois repetem o mesmo roteiro por meses sem reclamar e sem mudar nada.',
  'mutavel+mutavel': () => 'O que esfria: o assunto muda, o plano muda e a vontade muda junto, e falta a repetição que transforma atração em intimidade.',
  'cardeal+fixo': (c) => `O que esfria: ${c.md('cardeal')} propõe novidade e ${c.md('fixo')} quer o que já deu certo, e um lê o outro como pressa ou como marasmo.`,
  'cardeal+mutavel': (c) => `O que esfria: ${c.md('cardeal')} decide e ${c.md('mutavel')} se adapta, até o dia em que se adaptar sai caro demais.`,
  'fixo+mutavel': (c) => `O que esfria: ${c.md('fixo')} quer garantia e ${c.md('mutavel')} quer liberdade de mudar de ideia, e essa é a conversa que sempre volta pra cama.`,
};

// --- 9.2 CONVERSA ----------------------------------------------------------
const CONVERSA_FIGURA = {
  copresenca: (c) => `${c.A} e ${c.B} se entendem pela metade da frase, e o que ninguém puxa é justamente o defeito que os dois têm igual.`,
  trigono: (c) => `${c.A} e ${c.B} conversam sem esforço e sem tradução, e o assunto que ninguém puxa é o que exigiria discordar.`,
  sextil: (c) => `${c.A} e ${c.B} têm assunto fácil e nenhuma obrigação de concordar, e o que ninguém puxa é o combinado de fundo, porque está tudo funcionando.`,
  quadratura: (c) => `${c.A} e ${c.B} discutem bem: a conversa tem tese, contra-tese e placar — e o que ninguém puxa é o pedido de desculpa.`,
  oposicao: (c) => `${c.A} e ${c.B} conversam como dois lados da mesma questão, e cada frase de um responde a uma que o outro nem chegou a dizer — o que ninguém puxa é a pergunta de quem abre mão.`,
  aversao30: (c) => `${c.A} e ${c.B} não têm assunto pronto: a conversa precisa ser puxada, quase sempre pelo mesmo — e o que ninguém puxa é o assunto que exigiria explicar por que aquilo importa.`,
  aversao150: (c) => `${c.A} e ${c.B} partem de referências distantes e gastam boa parte do fôlego explicando o óbvio um pro outro, e o que ninguém puxa é o passado de cada um, que fica em caixas separadas.`,
};
...
const CONVERSA_ELEMENTO = {
  ...
  'fogo+água': (c) => `${c.el('fogo')} diz a coisa direta que ${c.el('água')} vai remoer por três dias, e o assunto reaparece na semana seguinte do nada.`,
...
const CONVERSA_MODALIDADE = {
  'cardeal+cardeal': () => 'Quem interrompe: os dois, o tempo todo, porque cada um já sabe aonde quer chegar.',
  'fixo+fixo': () => 'Quem interrompe: ninguém — os dois esperam a vez, mantêm a posição inteira, e a conversa fica educada sem mover nada de lugar.',
  'mutavel+mutavel': () => 'Quem interrompe: os dois, mas pra concordar e emendar outro assunto, e a conversa importante fica pro próximo domingo.',
  'cardeal+fixo': (c) => `Quem interrompe é ${c.md('cardeal')}, sempre com pressa de concluir, enquanto ${c.md('fixo')} repete o mesmo ponto até ser ouvido.`,
  'cardeal+mutavel': (c) => `Quem interrompe é ${c.md('cardeal')}, e ${c.md('mutavel')} deixa, muda de rota e só depois percebe que não foi ouvido.`,
  'fixo+mutavel': (c) => `${c.md('mutavel')} muda de assunto quando aperta e ${c.md('fixo')} volta ao assunto original na frase seguinte.`,
};

// --- 9.3 BRIGA -------------------------------------------------------------
const BRIGA_FIGURA = {
  copresenca: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é o defeito compartilhado: cada um enxerga no outro o que não suporta em si, e a volta acontece quando os dois cansam ao mesmo tempo.`,
  trigono: (c) => `O motivo recorrente entre ${c.A} e ${c.B} costuma ser pequeno e se dissolve no dia seguinte, porque nenhum dos dois tem gosto por manter conflito de pé.`,
  sextil: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é distância: nada explode, mas um dos dois some por uns dias e o outro não pergunta.`,
  quadratura: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é território — os dois querem mandar na mesma coisa e na mesma hora —, e a volta costuma vir por cansaço, não por acordo.`,
  oposicao: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é sempre o mesmo com nomes diferentes — o quanto cada um abre mão —, e a volta costuma ser rápida, porque nenhum dos dois se acostuma a ficar sem o contraponto.`,
  aversao30: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é mal-entendido puro: um disse uma coisa e o outro ouviu outra, e a volta depende de alguém explicar o que parecia óbvio.`,
  aversao150: (c) => `O motivo recorrente entre ${c.A} e ${c.B} é a sensação de não ser levado a sério, e a volta costuma vir de fora, por um assunto prático que obriga os dois a se falarem.`,
};
