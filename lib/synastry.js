...
// ---------------------------------------------------------------------------
// 5.1 O CAMINHO — todo par difícil sai com o que fazer, não só com o diagnóstico
// ---------------------------------------------------------------------------
...
// convivência". O `cuidado` das leituras tensas nomeia a dor com precisão e para
// ali; quem lê fecha a tela sabendo o que dói e nada sobre o que fazer com isso.
//
// A REGRA, e ela mora aqui e não na tela: caminho existe exatamente onde a
// CATEGORIA é tensa. Duas das quatro categorias são:
//   desarmonico ... quadratura e oposição (Tetrabiblos I.13)
//   semAspecto ... aversão de 30° e de 150° (Tetrabiblos I.16, e o degrau de
//                  baixo de IV.7, junto da oposição)
// Isso dá 84 dos 144 pares. As outras duas (harmonico, copresenca) NÃO recebem:
// o `cuidado` delas não é diagnóstico de dificuldade, e escrever "como conviver"
// onde não há atrito é encher linguiça com cara de conselho.
//
// A lista é EXPORTADA porque a tela e os testes precisam da mesma definição de
// "difícil" que o motor usa — a alternativa é a tela decidir sozinha quais
// categorias mostram a linha, e aí motor e tela discordam em silêncio.
// O texto de cada caminho vive nos packs (as três línguas), com as quatro regras
// dele escritas no cabeçalho das LEITURAS do pack PT.
export const CATEGORIAS_COM_CAMINHO = ['desarmonico', 'semAspecto'];

// ---------------------------------------------------------------------------
// 5.2 O ECO DO CAMINHO — o bloco 1 aponta, o bloco 2 entrega
// ---------------------------------------------------------------------------
// O caminho de 5.1 nasceu colado na Atenção, e a Atenção mora no BLOCO 2 — que
// a tela abre RECOLHIDO em toda leitura nova (é o que "depois a parte
// científica" quer dizer na prática). O efeito colateral só apareceu depois:
// quem não toca em "De onde vem isso" lê o diagnóstico inteiro e nunca vê a
// saída. O pedido do dono estava atendido pela metade — a saída existia nos três
// idiomas, testada, e não chegava aos olhos de quem não abre bibliografia.
//
// A correção é um ECO no bloco quente, e eco não é mudança de lugar: o caminho
// continua inteiro lá embaixo, com Lilly, o locus e a página. O que sobe é a
// PRIMEIRA frase — que é justamente onde mora o gesto, porque as regras 1 a 4 de
// 5.1 mandam abrir pela prática e fechar pela fonte.
//
// AS DUAS FUNÇÕES MORAM AQUI, E NÃO NA TELA, por dois motivos:
//   • o rótulo é texto de usuário, e texto de usuário vive nos packs — a tela
//     escrevendo "Por onde começar" à mão sairia em português pro mundo todo,
//     que é o defeito que a extração de 31/07/2026 corrigiu no resto da leitura;
//   • recortar a primeira frase é regra de CONTEÚDO, não de layout: se um dia o
//     caminho ganhar uma abertura diferente, quem conserta é este arquivo.
//
// E nenhuma das duas acrescenta campo ao objeto de sinastria(), de propósito:
// test/golden/synastry.pt.golden.json trava os campos da leitura e a lista
// CAMPOS_POSTERIORES_AO_GOLDEN tem UM item por decisão explícita. Eco é
// derivado do `caminho` que já existe — derivado não é campo novo.

// O rótulo do eco, no idioma da tela.
export function rotuloDoCaminho(lang = 'pt') {
  return packDoIdioma(lang).rotuloCaminho;
}

// A primeira frase do caminho, sem a abertura que o rótulo já diz.
//
// Todo caminho dos packs abre anunciando que dali em diante é prática ("Por onde
// começar, na prática: ...", "Por dónde empezar, en la práctica: ...", "Where to
// start, in practice: ..."). No eco essa abertura já está no rótulo do card, e
// repeti-la no corpo é dizer a mesma coisa duas vezes na mesma dobra.
//
// O corte é CONDICIONAL e conferido, não confiante: só acontece quando o trecho
// antes dos dois-pontos de fato começa pelo rótulo daquele idioma. Caminho que
// abrir de outro jeito sai inteiro, na primeira frase, sem eco quebrado — e o
// teste dos três idiomas avisa que a abertura mudou antes de qualquer deploy.
export function ecoDoCaminho(caminho, lang = 'pt') {
  if (!caminho || typeof caminho !== 'string') return null;
  const primeira = (caminho.match(/^[\s\S]*?[.!?](?=\s|$)/) || [caminho])[0].trim();
  const doisPontos = primeira.indexOf(':');
  if (doisPontos <= 0) return primeira;
  const abertura = primeira.slice(0, doisPontos);
  const rotulo = packDoIdioma(lang).rotuloCaminho;
  return abertura.startsWith(rotulo) ? primeira.slice(doisPontos + 1).trim() : primeira;
}
...
// Os intervalos de theme.js continuam canônicos em PT porque também são dado
// de compatibilidade. Na tela, porém, as abreviações de mês precisam seguir o
// idioma ativo — "23 Jul - 22 Ago" no meio do inglês denuncia uma tradução
// incompleta. Este helper só troca os rótulos; não altera nenhuma data.
const MESES_DO_SIGNO = {
  pt: { Jan: 'Jan', Fev: 'Fev', Mar: 'Mar', Abr: 'Abr', Mai: 'Mai', Jun: 'Jun', Jul: 'Jul', Ago: 'Ago', Set: 'Set', Out: 'Out', Nov: 'Nov', Dez: 'Dez' },
  es: { Jan: 'Ene', Fev: 'Feb', Mar: 'Mar', Abr: 'Abr', Mai: 'May', Jun: 'Jun', Jul: 'Jul', Ago: 'Ago', Set: 'Sep', Out: 'Oct', Nov: 'Nov', Dez: 'Dic' },
  en: { Jan: 'Jan', Fev: 'Feb', Mar: 'Mar', Abr: 'Apr', Mai: 'May', Jun: 'Jun', Jul: 'Jul', Ago: 'Aug', Set: 'Sep', Out: 'Oct', Nov: 'Nov', Dez: 'Dec' },
};

export function datasDoSigno(datasPt, lang = 'pt') {
  if (!datasPt) return datasPt;
  const idioma = String(lang || 'pt').split('-')[0];
  const meses = MESES_DO_SIGNO[idioma] || MESES_DO_SIGNO.pt;
  return String(datasPt).replace(/\b(Jan|Fev|Mar|Abr|Mai|Jun|Jul|Ago|Set|Out|Nov|Dez)\b/g, (mes) => meses[mes]);
}
...
export function sinastria(sa, sb, lang = 'pt') {
  ...
  return {
    ...
    // O CAMINHO PRÁTICO — presente nas leituras tensas, null nas outras. Ele
    // vem do pack junto com o resto da leitura (o spread acima já o trouxe);
    // esta linha existe pra que o campo SEMPRE exista no objeto, com null onde
    // não há caminho. Sem ela, `caminho` seria `undefined` em 60 dos 144 pares —
    // e `undefined` num join() de tela ou de teste vira a string "undefined".
    caminho: leitura.caminho || null,
