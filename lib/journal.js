...
// Salvar a leitura, anexar voz e anexar a reflexão são operações
// read-modify-write sobre a mesma lista. Sem uma fila, duas delas no mesmo
// instante podem ler a mesma versão antiga e a última gravação apaga o campo
// da primeira. A fila mantém as mutações locais na ordem em que foram pedidas.
let journalMutationQueue = Promise.resolve();

function serializedJournalMutation(operation) {
  const result = journalMutationQueue.then(operation, operation);
  journalMutationQueue = result.catch(() => undefined);
  return result;
}
...
async function readJson(key, fallback) {
  try {
    ...
    const valor = raw ? JSON.parse(raw) : fallback;
    // O TIPO tambem tem que bater (01/08/2026). Se o fallback e um array, o
    // que veio do storage PRECISA ser array — backup antigo, versao anterior
    // do app ou storage compartilhado podem devolver um objeto, e ai
    // entries.filter/.map estouram TypeError longe daqui, dentro de
    // toggleFavorito ou do corte de saveJournalEntry. Uma guarda no portao
    // vale por seis espalhadas.
...
async function saveJournalEntryUnlocked({
  type,
  typeLabel,
  title,
  body,
  ...
  const cleanCompletionId = typeof completionId === 'string' ? completionId.trim().slice(0, 180) : '';
  if (cleanCompletionId) {
    const existing = entries.find((candidate) => candidate.completionId === cleanCompletionId);
  ...
  if (cleanCompletionId) entry.completionId = cleanCompletionId;
  // Contexto privado e versão pública são campos opt-in e retrocompatíveis.
  // `question` e `readingDetails` nunca são usados pelo Feed. `shareBody` nasce
  // separado justamente para o botão social não precisar "tirar" texto íntimo
  // de um corpo pronto — privacidade por construção, não por regex.
  if (typeof shareBody === 'string' && shareBody.trim()) entry.shareBody = shareBody.trim();
  if (typeof question === 'string' && question.trim()) entry.question = question.trim();
  if (readingDetails && typeof readingDetails === 'object' && !Array.isArray(readingDetails)) {
    entry.readingDetails = readingDetails;
  }
...
export function saveJournalEntryWithStatus(fields) {
  return serializedJournalMutation(() => saveJournalEntryUnlocked(fields));
}

export async function saveJournalEntry(fields) {
  const result = await saveJournalEntryWithStatus(fields);
...
export function attachVoiceInsight(entryId, { voiceTranscript, aiInsight }) {
  return serializedJournalMutation(async () => {
    const entries = await readJson(JOURNAL_KEY, []);
...
// A nota escrita depois da revelação pertence à pessoa e fica local no Diário.
// Ela nunca entra em `shareBody`; compartilhar e refletir são ações separadas.
export function attachReflection(entryId, reflection) {
  return serializedJournalMutation(async () => {
    const entries = await readJson(JOURNAL_KEY, []);
    const clean = typeof reflection === 'string' ? reflection.trim().slice(0, 600) : '';
    let found = false;
    const updated = entries.map((entry) => {
      if (entry.id !== entryId) return entry;
      found = true;
      return { ...entry, reflection: clean || null };
    });
...
export async function getPinCredits() {
  try {
    // Era a UNICA leitura direta de AsyncStorage do modulo: addPinCredit e
    // pinEntry ja escreviam por setItemSeguro. Misturar os dois caminhos
    // significa que, quando o disco falha, a ESCRITA cai no fallback de
    // memoria e a LEITURA continua indo no disco quebrado — o credito some.
    const raw = await getItemSeguro(PIN_CREDITS_KEY);
...
export async function getActivePin() {
  ...
  // `until` invalido ou ausente dava NaN, e NaN < agora e FALSE — o pin era
  // dado como vigente PARA SEMPRE, fixando uma entrada no topo do Diario sem
  // nenhuma forma de expirar. Agora data ilegivel conta como expirada.
  const expira = new Date(pin.until).getTime();
  if (!Number.isFinite(expira) || expira < Date.now()) {
    try {
      await removeItemSeguro(PIN_KEY);
    } catch {}
    return null;
  }
  return pin;
...
// `t` vem da tela (useLanguage) em vez de a lib importar o dicionario: journal
// e o dono dos DADOS, e puxar i18n aqui criaria dependencia de apresentacao
// dentro da camada de persistencia. Sem `t` — chamada antiga, teste — devolve
// o portugues de sempre, byte a byte.
//
// Este e o caminho de ERRO do Insight da Semana: quando a IA nao responde, e
// isto que a pessoa le. Caminho de erro e o que ninguem revisa, porque nao
// aparece no uso normal — e era o unico texto do Diario ainda so em portugues.
export function getFallbackWeeklyInsight(entries, t) {
  const titles = entries.map((e) => `${e.typeLabel}: ${e.title}`).join('; ');
  if (typeof t === 'function') {
    return {
      title: t('diary.weekly.fallback.title'),
      body: t('diary.weekly.fallback.body', { titulos: titles }),
    };
  }
  return {
