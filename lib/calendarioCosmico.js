...
//
// TODO(i18n): todo o texto deste arquivo está em português e mora AQUI de
// propósito. lib/i18n.js está sendo editado por outro time neste momento e não
// pode ser tocado. Quando as chaves existirem, mover as strings para o prefixo
// `calendarioCosmico.` e ler por chave — a estrutura já está pronta pra isso
// (cada evento tem `titulo`, `paragrafo`, `fonte`, `tradicao`, `avisoDeIdade`
// em campos separados, nada concatenado na tela).
//
// ===========================================================================
...
const MARCOS_LUNARES = [
  {
    ...
    tradicao: {
      texto:
        'A lavoura romana plantava figueira, macieira, oliveira, pereira e videira na lua muda, o interlúnio, e à tarde.',
  ...
  },
  {
    ...
    tradicao: {
      texto:
        'Este é o trecho do mês com melhor lastro na lavoura antiga. Era na lua minguante que se cortava madeira, se colhia para secar, se tosquiava, se capinava e se estercava — tudo o que se queria que diminuísse em vez de crescer.',
...
function montaIngresso(data, signoAnterior, signoNovo) {
  ...
  const abertura =
    ...
    'É o pedaço de céu contado a partir do equinócio' +
    (cardinal ? `, e este ingresso é exatamente ${cardinal.evento} — ${cardinal.explica}` : '') +
    '. ';

  const recibo =
    MARCA_RECIBO +
    ' Cláudio Ptolomeu, Tetrabiblos I.22 (séc. II d.C.): os começos dos signos se contam dos equinócios e solstícios "and from no other source".';
...
const RETRO_INICIO = {
  ...
  paragrafo:
    'Mercúrio passa a andar para trás no céu — e "parece" é a palavra certa, porque é ilusão de perspectiva. ' +
  ...
  tradicao: {
    texto:
      'Ptolomeu nem classificava a retrogradação como boa ou má: para ele era uma fase térmica do ciclo do planeta — da primeira estação ao nascer acrônico, calor; do nascer acrônico à segunda estação, secura.',
    obra: 'Tetrabiblos I.8',
    autor: 'Cláudio Ptolomeu',
    seculo: 'séc. II d.C.',
  },
  avisoDeIdade:
    '"Mercúrio retrógrado quebra aparelho, derruba contrato e traz ex de volta" não está em fonte antiga nenhuma: é folclore do séc. XX/XXI, e a primeira menção de "Mercury retrograde" no New York Times é de 1996. Valente também não fala de Mercúrio em particular — a regra dele vale igual para Marte, Júpiter e Saturno.',
};
...
const RETRO_FIM = {
  ...
  paragrafo:
    'Mercúrio para e volta a andar para frente contra o fundo de estrelas. É a chamada segunda estação, e a partir do dia seguinte ' +
    'o movimento aparente é direto de novo. ' +
    MARCA_RECIBO +
    ' Vétio Valente, Anthologiae V (séc. II d.C.) — e a tradição tem aqui um final feliz que a internet costuma cortar fora: ' +
    'passada a segunda estação, os planetas "cancel any delay and reinstate the same activities".',
  fonte: 'Vétio Valente, Anthologiae V — séc. II d.C.',
  tradicao: {
    texto:
      'Lilly classificava a retrogradação como debilidade acidental, sinal de assunto que andava para trás e demorava a se resolver — nunca como catástrofe.',
    obra: 'Christian Astrology',
    autor: 'William Lilly',
    seculo: 'séc. XVII (1647/1659)',
  },
  avisoDeIdade:
    'A síntese honesta da tradição sobre retrogradação cabe em quatro palavras: demora, revisita, contradiz, destrava. Nada além disso está em fonte antiga.',
};
...
function montaAspecto(data, aspecto, orbeFinal) {
  ...
  const abertura = ehConjuncao
    ? `${planetA} e ${planetB} ficam hoje no mesmo ponto do círculo do zodíaco, vistos daqui da Terra — é isso que "conjunção" quer dizer: mesmo grau, um atrás do outro na nossa linha de visada. Ângulo é ângulo, dá pra conferir em qualquer efeméride, e o encontro tem hora marcada. `
    : `Vistos daqui da Terra, ${planetA} e ${planetB} ficam hoje a exatos ${grau}° um do outro sobre o círculo do zodíaco. Ângulo é ângulo: dá pra conferir em qualquer efeméride, e o instante tem hora marcada. `;
...
//   { ano, mes, ceuDisponivel, eventos, motivoIndisponivel }
...
// Atalho para o mês corrente — o caso mais comum de tela.
