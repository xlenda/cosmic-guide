...
//
// (6) TRÊS IDIOMAS, UM CANÔNICO — o PT mora AQUI e é ouro: as tabelas abaixo
//     são byte a byte o que sempre foram, e test/golden/quizCosmico.pt.golden
//     .json prova isso a cada rodada de teste. O QUE O GOLDEN PROVA, EXATAMENTE:
//     as 40 perguntas, as faixas de placar e as rodadas do dia foram capturadas
//     RODANDO este arquivo ANTES da extração — ali o golden é prova retroativa.
//     O bloco `chromeTela` do mesmo arquivo, não: ele foi escrito DEPOIS, a
//     partir do próprio CHROME_TELA, e por isso é trava daqui pra frente, nunca
//     prova de que o chrome não mudou ao sair da tela. Quem prova isso é o
//     literal escrito à mão em test/quizIdiomas.test.js, copiado do `TXT` de
//     screens/QuizCosmicoScreen.js — mesmo padrão de test/mitosIdiomas.test.js.
//     Espanhol e inglês vivem em lib/traducoes/quiz.es.js e quiz.en.js, com a
//     MESMA forma, e trocam SÓ texto: id, tema, base, a ordem das opções, o
//     `certaIdx` e a rodada determinística do dia continuam vindo daqui, em
//     qualquer língua. Chamada sem `lang` (ou com idioma desconhecido) devolve
//     PT — a compatibilidade com todo código antigo é total, e o teste cobra.
//     lib/i18n.js segue INTOCADO: até o chrome da tela viaja pelos packs (ver
//     CHROME_TELA), como em lib/emocoes.js.
//
// ===========================================================================
...
import PACK_QUIZ_ES from './traducoes/quiz.es';
import PACK_QUIZ_EN from './traducoes/quiz.en';

export const TAMANHO_RODADA = 7;
export const CHAVE_QUIZ_COSMICO = 'cosmic-quiz-cosmico-v1';

// Rótulos de exibição dos três temas, em PT canônico — es/en em temaRotulos().
...
// ---------------------------------------------------------------------------
// O CHROME DA TELA — as frases fixas de QuizCosmicoScreen
// ---------------------------------------------------------------------------
// Moradia nova, bytes velhos: estas strings viviam como constante PT local da
// tela e vieram pra cá SEM mudar um caractere (o golden prova), porque o idioma
// delas agora viaja pelos mesmos packs do conteúdo. lib/i18n.js segue intocado
// — mesma decisão de CHROME_TELA em lib/emocoes.js.
...
// ---------------------------------------------------------------------------
// IDIOMAS — os packs es/en por cima das tabelas PT
// ---------------------------------------------------------------------------
// Mesma arquitetura de lib/emocoes.js e lib/jornada.js. O que o pack pode
// trocar: pergunta, as 4 opções (NA MESMA ORDEM), explicação, fonte, rótulo de
// tema, frase de placar e chrome. O que o pack NUNCA vê: id, tema, base,
// certaIdx e a rodada do dia — por isso a resposta certa é a mesma alternativa
// em qualquer língua, e o dia 2026-07-31 sorteia as mesmas 7 perguntas no
// Brasil, no México e nos Estados Unidos.
const PACKS = { es: PACK_QUIZ_ES, en: PACK_QUIZ_EN };

// A forma canônica dos textos, num objeto só. O pack pt é DERIVADO das tabelas
// (nunca uma segunda cópia) e é contra ele que o teste de paridade compara as
// chaves e os tipos de es/en.
export function packDeTextos(lang = 'pt') {
  ...
  return {
    tela: { ...CHROME_TELA },
    temas: { ...TEMA_ROTULOS },
    placar: FAIXAS_PLACAR.map((f) => f.frase),
    perguntas,
  };
}

// Memo por idioma: as tabelas são estáticas, montar uma vez basta. Para 'pt' (e
// para idioma desconhecido) devolve a PRÓPRIA referência de PERGUNTAS —
// identidade, não cópia: a garantia mais forte de que o caminho PT não passou
// pelo refactor.
const _perguntasPorIdioma = {};

export function perguntasParaIdioma(lang = 'pt') {
  const pack = PACKS[lang];
  if (!pack) return PERGUNTAS;
  if (!_perguntasPorIdioma[lang]) {
    _perguntasPorIdioma[lang] = PERGUNTAS.map((p) => {
      const t = (pack.perguntas || {})[p.id] || {};
      const opcoes = Array.isArray(t.opcoes) && t.opcoes.length === p.opcoes.length ? t.opcoes : p.opcoes;
      return {
        ...p,
        // certaIdx, id, tema e base NÃO são tocados: vêm do spread acima.
        pergunta: t.pergunta || p.pergunta,
        opcoes: p.opcoes.map((o, i) => opcoes[i] || o),
        explicacao: t.explicacao || p.explicacao,
        fonte: t.fonte || p.fonte,
      };
    });
  }
  return _perguntasPorIdioma[lang];
}

export function temaRotulos(lang = 'pt') {
  const pack = PACKS[lang];
  return (pack && pack.temas) || TEMA_ROTULOS;
}
...
export function perguntaPorId(id, lang = 'pt') {
  return perguntasParaIdioma(lang).find((p) => p.id === id) || null;
}

// Todo texto que a pessoa pode ler nesta biblioteca, como [caminho, texto] — a
// matéria-prima das varreduras. Sem `lang` a saída é EXATAMENTE a de antes do
// refactor de idiomas; o chrome entra separado (varredura própria no teste),
// pra manter esta lista comparável com a suíte PT antiga.
export function textosVisiveis(lang = 'pt') {
  ...
  frasesDePlacar(lang).forEach((f, i) => saida.push([`placar[${i}]`, f]));
  return saida;
}
...
// O IDIOMA NÃO ENTRA NO SORTEIO: a semente é só o dia, então quem abre o app em
// espanhol recebe as MESMAS sete perguntas de quem abre em português — traduzir
// não pode mudar a rodada, senão duas pessoas param de poder comparar o dia.
export function rodadaDoDia(dia = localDayStr(), lang = 'pt') {
  const banco = perguntasParaIdioma(lang);
  const ordem = embaralharIndices(PERGUNTAS.length, hashStr(`quiz-cosmico:${dia}`));
  return ordem.slice(0, TAMANHO_RODADA).map((i) => banco[i]);
}
...
// As frases de placar no idioma pedido, na MESMA ordem de FAIXAS_PLACAR — as
// faixas (os cortes de 1, 5/7 e 3/7) são estruturais e vivem só aqui.
export function frasesDePlacar(lang = 'pt') {
  const pack = PACKS[lang];
  const doPack = pack && Array.isArray(pack.placar) && pack.placar.length === FAIXAS_PLACAR.length
    ? pack.placar
    : null;
  return FAIXAS_PLACAR.map((f, i) => (doPack ? doPack[i] || f.frase : f.frase));
}

export function fraseDoPlacar(acertos, total = TAMANHO_RODADA, lang = 'pt') {
  const t = total > 0 ? total : TAMANHO_RODADA;
  const razao = Math.max(0, Math.min(1, (acertos || 0) / t));
  const i = FAIXAS_PLACAR.findIndex((f) => razao >= f.min);
  const escolhida = i >= 0 ? i : FAIXAS_PLACAR.length - 1;
  return frasesDePlacar(lang)[escolhida];
}
